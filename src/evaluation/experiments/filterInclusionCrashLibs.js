...
const resultsDirectory = baseDir + config.resultsDirectory + '/filter-inclusion-crash/'; // The directory where the final  global writes go
...
function filterInclusionCrashingLibs(allLibraries, jobQueue) {
    ...
    for (let i = 0; i < allLibraries.length; i++) {
        ...
        if (!pathExists(validationTest.resultFilePath)) {
            validationTest.libraryPaths[libraryName] = libraries.libraryByName(libraryName).libraryFiles[0]; // Currently considering the first file
...
function writeCrashingLibs(jobQueue) {
    ...
    for (let i = 0; i < resultFiles.length; i++) {
        ...
        let rawresults = JSON.parse(fs.readFileSync(resultsDirectory + "/" + resultFile, {encoding: "utf8"}));
    ...
    fs.writeFileSync(config.singleLibCrashingFile, JSON.stringify([...crashingLibs]));
...
function createJobs(jobQueue) {
    ...
    jobQueue.newJob(jobID, [new RegExp(filterInclusionCrashJobIdPrefix)], writeCrashingLibs.bind(null, jobQueue));
}
