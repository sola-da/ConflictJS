...
(function () {
    ...
    const browser = require("../../utilities/browser"), libraries = require("../../utilities/libraries"),
    ...
    const resultFileName = require('../../utilities/constructFileName').constructUniqueFilename;

    /* Misc */
    const findPotentialConflictsJobID = require('./findPotentialConflicts').jobID;

// ----------------------------------------- Experiments to validate conflict --------------------------------
    const inclusionTest = require('../validation-tests/inclusionTest').inclusionTest;
    const compareType = require('../validation-tests/compareType').compareType;
    const compareNonFunctions = require('../validation-tests/compareNonFunctions').compareNonFunctions;
    const compareFunctions = require('../validation-tests/compareFunctions').compareFunctions;
// -----------------------------------------------------------------------------------------------------------
        ...
        accessPaths.forEach(accessPath => {
            ...
            for (let i = 0; i < pairsOfLibraries.length; i++) {
                ...
                pairsOfLibraries[i].forEach(lib => {
                    validationTest.libraryPaths[lib] = libraries.libraryByName(lib).libraryFiles[0]; // Currently considering the first file
                });
                ...
                if (validated && conflictingJob) {
                    ...
                    if (validationTest.runTests.has("inclusionTest") && !validationTest.runTests.has("typeTest")) { // inclusion test done
                        // Experiment Two: Type test
                        validationTest.name = "typeTest";
                        compareType(validationTest, jobQueue, dependence);
                    }

                    if (validationTest.runTests.has("typeTest")) { // type test done
                        // Experiment Three: Value test
                        if (!accessPathFunctionType(potentialConflicts[accessPath], validationTest.libraryNames)) {
    ...
    function writeValidatedResults(validationTest, job) {
        let result = JSON.parse(fs.readFileSync(validationTest.resultFilePath, {encoding: "utf8"}));
        ...
        if (pathExists(validatedResultFile)) {
            results = JSON.parse(fs.readFileSync(validatedResultFile, {encoding: "utf8"}));
        }
    ...
    function checkValidationStatus(validationTest) {
        ...
        if (pathExists(result_File)) {
            let result = JSON.parse(fs.readFileSync(result_File, {encoding: "utf8"}));
            for (let job in result) {
                if (result.hasOwnProperty(job)) {
                    ...
                    if (inclusionPattern.test(job)) validationTest.runTests.add('inclusionTest');
                    if (typePattern.test(job)) validationTest.runTests.add('typeTest');
                    if (valuenonfuncPattern.test(job)) validationTest.runTests.add('value-test-non-functions');
                    if (valuefuncPattern.test(job)) validationTest.runTests.add('value-test-functions');
    ...
    function readPotentialConflicts() {
        ...
        let rawPotentialConflicts = fs.readFileSync(potentialConflictsFile, {encoding: "utf8"});
