    ...
    if (!testsGenerated) {
        ...
        if (messages.size > 1) {
            messages.forEach(function (val, key) {
                let tests = [];
                try {
                    tests = testGeneration.generateTests([new testGeneration.APIObject("window", val, [accessPath])], NO_OF_TESTS, NO_OF_CALLS, true);
                } catch (err) {
        ...
        } else if (testSizes == 1) { // Tests could not be generated for one of the library
        ...
        messages.forEach(function (val, key) {
            if (key === 'ERROR') {
                ...
                result_content = key /*+ ' no. of generated tests is ' + generatedTestsSize*/;
            }
            else
                testsgen = []; // Empty the generated tests if there is no error
        });
        window.setTimeout(sendBackToServer, 100);
    }
}