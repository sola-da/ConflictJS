...
(function () {
    ...
    function inclusionTest(validationTest, jobQueue, dependence) {
        ...
        hostingPageClient = "\t<script src='../src/evaluation/validation-tests/inclusionTest_Hostingclient.js'></script>";

        // LibraryClientCode = "\n\t<script>var message = '" + validationTest.libraryNames + "';</script>";
        LibraryClientCode = "\t<script src='../src/evaluation/validation-tests/inclusionTest_Libclient.js'></script>";
        htmlgen.generateTemplate(validationTest, hostingPageClient, LibraryClientCode);
