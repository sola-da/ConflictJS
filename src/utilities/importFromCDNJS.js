...
(function () {
    ...
    function importAsBenchmarks(maxLibraries) {
        let cdnLibraries = JSON.parse(fs.readFileSync(cdnJSFile, {encoding: "utf8"})).results;
        let imported = 0;
        for (let i = 0; i < cdnLibraries.length; i++) {
            ...
            if (library.latest.match(/.js$/) !== null) {
                ...
                fs.writeFileSync(libraryDir + "/libraryInfo.json", JSON.stringify(libraryInfo, 0, 2));
                imported++;
            } else {
