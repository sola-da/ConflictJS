...
(function () {
    ...
    function startServer(jobQueue) {
        ...
        app.post('/reportResult', function (request, response) {
            ...
            if (result.generatedTests.length !== 0) { // Write only if the tests are generated
                ...
                fs.writeFileSync(generatedTestsDir + result.typeofTest + '.json', JSON.stringify(result.generatedTests, 0, 2));
            }
            let summarisedResult = {};

            let resultfile = result.resultFilePath;
            if (pathExists(resultfile)) {
                summarisedResult = JSON.parse(fs.readFileSync(resultfile, {encoding: "utf8"}));
            }
            ...
            gwpattern.test(typeofTest) ? summarisedResult = result.content : summarisedResult[typeofTest] = result.content;
