    ...
    function ($window) {
      ...
      return {
        ...
        link: function (scope, element, attrs) {
          ...
          function renderPDF() {
            clearCanvas();
            var params = { url: url, withCredentials: creds };
            httpHeaders && (params.httpHeaders = httpHeaders),
              url &&
                ...
                pdfLoaderTask.then(
                  function (_pdfDoc) {
                    "function" == typeof scope.onLoad && scope.onLoad(),
                      (pdfDoc = _pdfDoc),
          ...
          var url = scope.pdfUrl,
            ...
            pdfDoc = null,
            pageToDisplay = isFinite(attrs.page) ? parseInt(attrs.page) : 1,
            pageFit = "page-fit" === attrs.scale,
            scale = attrs.scale > 0 ? attrs.scale : 1,
            canvasid = attrs.canvasid || "pdf-canvas",
            canvas = document.getElementById(canvasid);
          debug = attrs.hasOwnProperty("debug") ? attrs.debug : !1;
          ...
          windowEl.on("scroll", function () {
            ...
            (scope.renderPage = function (num) {
              renderTask && renderTask._internalRenderTask.cancel(),
                pdfDoc.getPage(num).then(function (page) {
                  ...
                  (viewport = page.getViewport(scale)),
                    ...
                    renderTask.promise
                      .then(function () {
                        "function" == typeof scope.onPageRender &&
                          scope.onPageRender();
                      })
            ...
            (scope.goNext = function () {
              scope.pageToDisplay >= pdfDoc.numPages ||
                ((scope.pageToDisplay = parseInt(scope.pageToDisplay) + 1),
                (scope.pageNum = scope.pageToDisplay));
            }),
            ...
            (scope.fit = function () {
              (pageFit = !0), scope.renderPage(scope.pageToDisplay);
            }),
            (scope.changePage = function () {
              scope.renderPage(scope.pageToDisplay);
            }),
            (scope.rotate = function () {
              "rotate0" === canvas.getAttribute("class")
                ? canvas.setAttribute("class", "rotate90")
                : "rotate90" === canvas.getAttribute("class")
                ? canvas.setAttribute("class", "rotate180")
                : "rotate180" === canvas.getAttribute("class")
                ? canvas.setAttribute("class", "rotate270")
                : canvas.setAttribute("class", "rotate0");
            }),
            scope.$watch("pageNum", function (newVal) {
              (scope.pageToDisplay = parseInt(newVal)),
                null !== pdfDoc && scope.renderPage(scope.pageToDisplay);
            }),
