  ...
  window.Ember.Resource.sendEvent = (function () {
    if (Ember.sendEvent.length === 2) {
      return function sendEvent(obj, eventName, params, actions) {
        Ember.warn(
          "Ember.Resources.sendEvent can't do anything with actions on Ember 0.9",
          !actions
        );
        params = params || [];
...
(function (exports) {
  var Ember = exports.Ember,
    NullTransport = { subscribe: Ember.K, unsubscribe: Ember.K };
  ...
  var RemoteExpiry = Ember.Mixin.create({
    ...
    subscribeForExpiry: function () {
      ...
      Ember.Resource.PushTransport.subscribe(
        remoteExpiryScope,
    ...
    willDestroy: function () {
      ...
      Ember.Resource.PushTransport.unsubscribe(
        remoteExpiryScope,
        this._expiryCallback
      );
    ...
    stale: function (updatedAt) {
      return (
        ...
        +this.get("expiryUpdatedAt") < +updatedAt
      );
    },
  });
...
(function () {
  Ember.Resource.IdentityMap = function (limit) {
    this.cache = new LRUCache(
      limit || Ember.Resource.IdentityMap.DEFAULT_IDENTITY_MAP_LIMIT
    );
  };
...
(function (exports) {
  ...
  exports.Ember.Resource.ajax = function (options) {
    ...
    if (options.error) {
      ...
      options.error = errorHandlerWithContext(
        window.Ember.Resource.errorHandler,
        options
      );
    }
    var dfd = $.Deferred();
...
(function (exports) {
  ...
  Ember.Resource.deepMerge = function (objA, objB) {
    ...
    for (var key in objB) {
      if (objB.hasOwnProperty(key)) {
        ...
        if (
          Ember.typeOf(newValue) === "object" &&
          Ember.typeOf(oldValue) === "object"
        ) {
  ...
  Ember.Resource.SchemaItem.reopenClass({
    create: function (name, schema) {
      ...
      if (type) {
        if (type.isEmberResource || isString(type)) {
  ...
  Ember.Resource.AttributeSchemaItem = Ember.Resource.AbstractSchemaItem.extend(
    {
      ...
      setValue: function (instance, value) {
        ...
        if (
          value !== null &&
          value !== undefined &&
          Ember.typeOf(value.toJSON) == "function"
        ) {
          value = value.toJSON();
        }
    ...
    create: function (name, schema) {
      ...
      if (this === Ember.Resource.AttributeSchemaItem) {
        switch (definition.type) {
          case Number:
            return Ember.Resource.NumberAttributeSchemaItem.create(
              name,
              schema
            );
          case String:
            return Ember.Resource.StringAttributeSchemaItem.create(
              name,
              schema
            );
          case Boolean:
            return Ember.Resource.BooleanAttributeSchemaItem.create(
              name,
              schema
            );
          case Date:
  ...
  Ember.Resource.NumberAttributeSchemaItem =
    Ember.Resource.AttributeSchemaItem.extend({
      theType: Number,
  ...
  Ember.Resource.StringAttributeSchemaItem =
    Ember.Resource.AttributeSchemaItem.extend({
      theType: String,
      typeCast: function (value) {
  ...
  Ember.Resource.BooleanAttributeSchemaItem =
    Ember.Resource.AttributeSchemaItem.extend({
      theType: Boolean,
      typeCast: function (value) {
        if (
  ...
  Ember.Resource.DateAttributeSchemaItem =
    Ember.Resource.AttributeSchemaItem.extend({
      theType: Date,
      typeCast: function (value) {
        if (
  ...
  Ember.Resource.HasOneNestedSchemaItem =
    Ember.Resource.HasOneSchemaItem.extend({
      getValue: function (instance) {
        var data = this.data(instance);
        if (!data) return;
        ...
        if (value) {
          value = (this.get("parse") || type.parse).call(
            type,
            Ember.copy(value)
          );
  ...
  Ember.Resource.HasOneNestedSchemaItem.reopenClass({
    create: function (name, schema) {
      ...
      if (!schema[id_name]) {
        ...
        schema[id_name] = Ember.Resource.HasOneNestedIdSchemaItem.create(
          id_name,
          schema
        );
      }
  ...
  Ember.Resource.HasOneNestedIdSchemaItem =
    Ember.Resource.AbstractSchemaItem.extend({
      theType: Number,
      getValue: function (instance) {
  ...
  Ember.Resource.HasOneRemoteSchemaItem =
    Ember.Resource.HasOneSchemaItem.extend({
      getValue: function (instance) {
        var data = this.data(instance);
        if (!data) return;
  ...
  Ember.Resource.HasManyRemoteSchemaItem =
    Ember.Resource.HasManySchemaItem.extend({
      dependencies: ["id", "isInitializing"],
      getValue: function (instance) {
        if (getPath(instance, "isInitializing")) return;
        var options = { type: this.get("itemType") };
  ...
  Ember.Resource.HasManyNestedSchemaItem =
    Ember.Resource.HasManySchemaItem.extend({
      getValue: function (instance) {
        var data = this.data(instance);
        if (!data) return;
        ...
        var options = { type: this.get("itemType"), content: data };
        if (this.get("parse")) options.parse = this.get("parse");
        return this.get("type").create(options);
      },
      setValue: function (instance, value) {},
  ...
  Ember.Resource.HasManyInArraySchemaItem =
    Ember.Resource.HasManySchemaItem.extend({
      getValue: function (instance) {
        var data = this.data(instance);
        if (!data) return;
  ...
  Ember.Resource.Lifecycle = {
    ...
    clock: Ember.Object.create({
      ...
      start: function () {
        this.stop();
        Ember.Resource.Lifecycle.clock.set(
          "timer",
          setInterval(Ember.Resource.Lifecycle.clock.tick, 1e4)
        );
      },
      stop: function () {
    ...
    classMixin: Ember.Mixin.create({
      create: function (options, data) {
        ...
        var instance = this._super.apply(this, arguments);
        if (
          getPath(instance, "resourceState") ===
    ...
    prototypeMixin: Ember.Mixin.create({
      ...
      init: function () {
        ...
        var updateExpiry = function () {
          ...
          expireAt.setSeconds(
            expireAt.getSeconds() + getPath(self, "expireIn")
          );
        ...
        Ember.addListener(this, "didSave", this, function () {
          set(
            self,
            "resourceState",
            resourceStateBeforeSave || Ember.Resource.Lifecycle.UNFETCHED
          );
        });
      },
      isFetchable: Ember.computed(function (key, value) {
        ...
        return (
          state == Ember.Resource.Lifecycle.UNFETCHED || this.get("isExpired")
        );
      }).volatile(),
      isInitializing: Ember.computed("resourceState", function (key, value) {
        return (
          (getPath(this, "resourceState") ||
            Ember.Resource.Lifecycle.INITIALIZING) ===
          Ember.Resource.Lifecycle.INITIALIZING
        );
      }).cacheable(),
      isFetching: Ember.computed("resourceState", function (key, value) {
        return (
          getPath(this, "resourceState") === Ember.Resource.Lifecycle.FETCHING
        );
      }).cacheable(),
      isFetched: Ember.computed("resourceState", function (key, value) {
        return (
          getPath(this, "resourceState") === Ember.Resource.Lifecycle.FETCHED
        );
      }).cacheable(),
      ...
      isSaving: Ember.computed("resourceState", function (key, value) {
        return (
          getPath(this, "resourceState") === Ember.Resource.Lifecycle.SAVING
        );
      }).cacheable(),
  ...
  Ember.Resource.reopen(
    {
      ...
      updateWithApiData: function (json) {
        var data = getPath(this, "data"),
          parsedData;
        if (!data) {
          return;
        }
        parsedData = this.constructor.parse(json);
        if (!this.isFresh(parsedData)) {
          return;
        }
      ...
      fetch: function (ajaxOptions) {
        ...
        if (!getPath(this, "isFetchable"))
          return $.when(this.get("data"), this);
      ...
      destroyResource: function () {
        var previousState = getPath(this, "resourceState"),
          self = this;
  ...
  Ember.Resource.reopenClass(
    {
      ...
      create: function (options, data) {
        ...
        if (klass === this) {
          var instance;
          var createWithMixins =
            this.superclass.createWithMixins || this._super;
          var id = data.id || options.id;
          if (id && !options.skipIdentityMap && this.useIdentityMap) {
            this.identityMap =
              this.identityMap ||
              new Ember.Resource.IdentityMap(this.identityMapLimit);
            id = id.toString();
      ...
      define: function (options) {
        ...
        var klass = this.extend(
          createSchemaProperties(schema),
          Ember.Resource.RemoteExpiry
        );
        var classOptions = { schema: schema };
        if (this !== Ember.Resource) {
  ...
  Ember.ResourceCollection = Ember.ArrayProxy.extend(
    Ember.Resource.RemoteExpiry,
    {
      ...
      fetch: function (ajaxOptions) {
        if (this.deferredFetch && !getPath(this, "isExpired"))
          return this.deferredFetch.promise();
        if (!getPath(this, "isFetchable") || getPath(this, "prePopulated"))
          return $.when(this);
        var self = this;
        Ember.Resource.sendEvent(self, "willFetch");
        var result = (this.deferredFetch = $.Deferred()),
  ...
  Ember.ResourceCollection.reopenClass(
    {
      isEmberResourceCollection: true,
      identityMapLimit:
        Ember.Resource.IdentityMap.DEFAULT_IDENTITY_MAP_LIMIT * 5,
      useIdentityMap: true,
      create: function (options) {
        ...
        if (!options.prePopulated && options.url && this.useIdentityMap) {
          ...
          instance =
            this.identityMap.get(options.id) ||
            createWithMixins.call(this, options);
