      ...
      function (require, module, exports) {
        ...
        var FLACDecoder = AV.Decoder.extend(function () {
          ...
          const SAMPLE_RATES = new Int32Array([
            0, 88200, 176400, 192e3, 8e3, 16e3, 22050, 24e3, 32e3, 44100, 48e3,
            96e3, 0, 0, 0, 0,
          ]);
          ...
          this.prototype.readChunk = function () {
            ...
            if ((stream.read(15) & 32767) !== 32764)
              throw new Error("Invalid sync code");
            var isVarSize = stream.read(1),
              bsCode = stream.read(4),
              srCode = stream.read(4),
              chMode = stream.read(4),
              bpsCode = stream.read(3);
            stream.advance(1);
            this.chMode = chMode;
            var channels;
            ...
            else if (srCode === 12) sampleRate = stream.read(8) * 1e3;
          ...
          this.prototype.decodeSubframe = function (channel) {
            ...
            if (type === 0) {
              ...
              this.decode_subframe_fixed(channel, type & ~8);
            } else if (type >= 32) {
              this.decode_subframe_lpc(channel, (type & ~32) + 1);
            } else {
          ...
          function multiply_add(total, a, b) {
            ...
            var a48 = total[1] >>> 16;
            var a32 = total[1] & 65535;
            var a16 = total[0] >>> 16;
            var a00 = total[0] & 65535;
            var b48 = r_high >>> 16;
            var b32 = r_high & 65535;
            var b16 = r_low >>> 16;
            var b00 = r_low & 65535;
            var c48 = 0,
              c32 = 0,
          ...
          function clz(input) {
            ...
            if (!(curbyte & 240)) output += 4;
            else curbyte >>>= 4;
            if (curbyte & 8) return output;
            if (curbyte & 4) return output + 1;
            if (curbyte & 2) return output + 2;
            if (curbyte & 1) return output + 3;
            return output + 4;
          }
        });
            ...
            while (stream.available(1) && !this.last) {
              ...
              switch (this.type) {
                case STREAMINFO:
                  ...
                  this.emit(
                    "duration",
                    ((sampleCount / this.format.sampleRate) * 1e3) | 0
                  );
                  stream.advance(16);
                ...
                case SEEKTABLE:
                  for (var s = 0; s < this.size / 18; s++) {
                    if (
                      stream.peekUInt32(0) == 4294967295 &&
                      stream.peekUInt32(1) == 4294967295
                    ) {
                      stream.advance(18);
                    } else {
