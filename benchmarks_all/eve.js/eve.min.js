...
(function (u) {
  function g(a) {
    if (!f)
      a: if (!f) {
        for (
          var b = ["jQuery", "MooTools", "YUI", "Prototype", "dojo"], d = 0;
          d <= b.length;
          d++
        )
          if (window[b[d]]) {
  ...
  function l(a, b) {
    if (window.console) {
      var d = m;
      if (!m) for (var d = !1, c = 0; c < n.length; c++) n[c] == a && (d = !0);
      if (d) {
        for (; 10 > a.length; ) a += " ";
        a = a.substring(0, 10) + " - ";
        console.info(a, b);
  ...
  function q(a, b, d, c) {
    for (var e in r) b[e] = r[e];
    for (e in p) b[e] = p[e];
    g("YUI")
      ? YUI().use("node", function (e) {
          j = e.one;
          d[c] = a.apply(b);
        })
      : g("dojo")
      ? require(["dojo/NodeList-dom", "dojo/NodeList-traverse"], function (e) {
          j = e;
          d[c] = a.apply(b);
        })
      : (d[c] = a.apply(b));
  ...
  u.Eve = {
    ...
    register: function (a, b) {
      l(a, "registered");
      if (h[a]) throw Error("Module already exists: " + a);
      h[a] = b;
      return this;
    },
    extend: function (a, b) {
    ...
    scope: function (a, b) {
      ...
      q(b, { name: a, namespace: a }, s, a);
    },
    attach: function (a, b) {
      var d = [],
        c = 0;
      for (c; c < arguments.length; c++) d[d.length] = arguments[c];
      l(a, "attached to " + b);
      if (t[a + b]) return !1;
      if (!h[a]) return console.warn("Module not found: " + a), !1;
      q(
        function () {
  ...
  var r = {
    listen: function (a, b, d) {
      ...
      d || ((d = b), (b = a), (a = ""));
      a = a || "";
      var e = this.event ? this.find() : document.body,
        v = this.name,
        f = (this.namespace + " " + a).trim(),
        h = {},
        k;
      for (k in this) this.hasOwnProperty(k) && (h[k] = this[k]);
      if (g("jQuery")) $(e).delegate(f, b, c);
      else if (g("MooTools")) $(e).addEvent(b + ":relay(" + f + ")", c);
      else if (g("YUI")) j(e).delegate(b, c, f);
      else if (g("Prototype")) $(e).on(b, f, c);
      else
        g("dojo") &&
          require(["dojo/on"], function (a) {
            a(e, f + ":" + b, c);
          });
    },
    find: function (a) {
