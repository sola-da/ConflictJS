(function (window) {
  ...
  findTemplate = function (key, setOnMissing) {
    assert(
      "You must provide a translation key string, not %@".fmt(key),
      typeof key === "string"
    );
    ...
    if (typeof PlainHandlebars.compile === "function") {
      ...
      return function cannotCompileTemplate() {
        throw new Ember.Error(
          "The default Ember.I18n.compile function requires the full Handlebars. Either include the full Handlebars or override Ember.I18n.compile."
        );
      };
    }
  }
  ...
  var TranslationView = Ember._MetamorphView.extend({
    ...
    wrappingTagName: Ember.computed(function (propertyName, newValue) {
      if (arguments.length > 1 && newValue != null) {
        return newValue;
      }
      ...
      if (Ember.FEATURES.hasOwnProperty("I18N_TRANSLATE_HELPER_SPAN")) {
        ...
        Ember.deprecate(
          "The {{t}} helper will no longer use a <span> tag in future versions of Ember.I18n. Set Ember.FEATURES.I18N_TRANSLATE_HELPER_SPAN to false to quiet these warnings and maintain older behavior."
        );
    ...
    render: function (buffer) {
      ...
      if (wrappingTagName) {
        buffer.push("<" + wrappingTagName + ' id="' + uniqueElementId() + '">');
      }
      buffer.push(text);
      if (wrappingTagName) {
        buffer.push("</" + wrappingTagName + ">");
      }
    },
  });
  EmHandlebars.registerHelper("t", function (key, options) {
    ...
    if (options.types[0] !== "STRING") {
      warn(
        "Ember.I18n t helper called with unquoted key: %@. In the future, this will be treated as a bound property, not a string literal.".fmt(
          key
        )
      );
    }
    Ember.deprecate(
      "Passing a tagName to the {{t}} helper is deprecated and will be removed in v3.0.",
      tagName == null
    );
    ...
    Ember.keys(attrs).forEach(function (property) {
      ...
      var _ref = [normalized.root, normalized.path],
        root = _ref[0],
