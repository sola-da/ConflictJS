...
define("ember-dialog/app", [
  "ember-dialog/initializers/dialog-manager",
  "exports",
], function (__dependency1__, __exports__) {
  "use strict";
  var DialogManagerInitializer = __dependency1__["default"];
  var App = Ember.Application.extend();
  App.initializer(DialogManagerInitializer);
  __exports__["default"] = App;
});
define("ember-dialog/initializers/dialog-manager", [
  "ember-dialog/services/dialog-manager",
  "ember-dialog/components/dialog",
  "exports",
], function (__dependency1__, __dependency2__, __exports__) {
  "use strict";
  var DialogManager = __dependency1__["default"];
  var DialogComponent = __dependency2__["default"];
  ...
  __exports__["default"] = { name: "dialog-manager", initialize: initialize };
});
define("ember-dialog/services/dialog-manager", ["exports"], function (
  __exports__
) {
  "use strict";
  __exports__["default"] = Ember.Object.extend(Ember.Evented, {
    rootElement: "body",
    defaults: { ms: 5e3, decline: "decline", accept: "accept" },
    dialogData: {
      acceptLabel: "yes",
      declineLabel: "no",
    ...
    alert: function (view, controller, dialogData) {
      ...
      dialogData = Ember.$.extend({}, this.get("dialogData"), dialogData, {
        className: ["alert", className].join(" "),
      });
      return this._openWithLayout(
        "dialogs/alert",
        view,
        controller,
        dialogData
    ...
    confirm: function (view, controller, dialogData) {
      ...
      var className = dialogData.className || "";
      dialogData = Ember.$.extend({}, this.get("dialogData"), dialogData, {
        className: ["confirm", className].join(" "),
      });
      return this._openWithLayout(
        "dialogs/confirm",
        view,
    ...
    custom: function (view, controller, dialogData) {
      ...
      var className = dialogData.className || "";
      dialogData = Ember.$.extend({}, this.get("dialogData"), dialogData, {
        className: ["custom", className].join(" "),
      });
      return this._openWithLayout(
        "dialogs/custom",
        view,
    ...
    notice: function (view, ms, dialogData) {
      ...
      var className = dialogData.className || "";
      dialogData = Ember.$.extend(
        { name: name },
        this.get("dialogData"),
        dialogData,
        {
          className: ["notice", "highest", className].join(" "),
          substrate: false,
        }
      );
      var promise = this._openWithLayout(
          "dialogs/notice",
          view,
          null,
          dialogData,
    ...
    open: function (name, view, controller, dialogData, setActive) {
      var container = controller.container || this.container,
        dialog = this.getDialog(name),
        rootElement =
          (controller.namespace && controller.namespace.rootElement) ||
          this.get("rootElement"),
        template,
        data = { name: name };
      Ember.merge(data, dialogData || {});
      Ember.ENV.LOG_DIALOG &&
        Ember.Logger.log(
          "%cDialogManager:%c Opening dialog named %s",
          "font-weight: 900;",
          null,
          name
        );
      if (Ember.typeOf(dialog) === "undefined") {
        Ember.ENV.LOG_DIALOG &&
          Ember.Logger.log(
            "%cDialogManager:%c Creating dialog named %s",
            "font-weight: 900;",
            null,
            name
          );
        dialog = container
          .lookupFactory("component:dialog")
          .createWithMixins(data);
        if (
          !Ember.isEqual(
            data.declineHandlerName,
            this.get("defaults.decline")
          ) ||
          ...
          if (
            ...
            actions[data.declineHandlerName] = function (dialog) {
              this.decline();
            };
          }
          if (
            ...
            actions[data.acceptHandlerName] = function (dialog) {
              this.accept();
            };
          }
      ...
      setActive && this.get("dialogsList").pushObject(name);
      return dialog.show();
    },
    close: function (name) {
      this.get("controllers:" + name).reopen({
        target: this.get("targets:" + name),
      });
      return new Ember.RSVP.Promise(
        Ember.run.bind(this, function (resolve, reject) {
          var dialogsList = this.get("dialogsList");
          this.set("dialogsList", dialogsList.without(name));
          var dialog = this.getDialog(name);
          Ember.ENV.LOG_DIALOG &&
            Ember.Logger.log(
              "%cDialogManager:%c Closing dialog named %s",
              "font-weight: 900;",
              null,
              name
            );
    ...
    setDialog: function (name, instance) {
      Ember.ENV.LOG_DIALOG &&
        Ember.Logger.log(
          "%cDialogManager:%c Register dialog named %s",
          "font-weight: 900;",
          null,
          name
        );
    ...
    _destroyDialog: function (name) {
      var dialog =
        Ember.typeOf(name) === "string" ? this.getDialog(name) : name;
      if (dialog) {
        dialog.destroy();
      }
      return this;
    ...
    _getView: function (view, controller) {
      ...
      if (Ember.typeOf(view) === "string") {
        ...
        throw new Ember.Error(
          "The given view unrecognized. Should be a text or name of template or view class."
        );
      }
      return view;
    },
    _openWithLayout: function (
      layoutName,
      view,
      controller,
      dialogData,
      ...
      var dialog,
        promise,
        name =
          Ember.typeOf(dialogData.name) === "string"
            ? dialogData.name
            : this._generateDialogName();
      if (
        !(controller instanceof Ember.Controller) &&
        Ember.typeOf(controller) === "object"
      ) {
      ...
      controller.reopen({
        target: dialog,
        dialog: Ember.computed.alias("target").readOnly(),
      });
      promise = new Ember.RSVP.Promise(function (resolve, reject) {
        dialog.resolved = function () {
          resolve && resolve();
        };
        dialog.rejected = function () {
          reject && reject();
        };
      });
      return promise;
...
define("ember-dialog/components/dialog", [
  ...
  __exports__["default"] = Ember.Component.extend({
    ...
    decline: function () {
      Ember.ENV.LOG_DIALOG &&
        Ember.Logger.log(
          "✘ %cDecline action%c: " + this.get("name"),
    ...
    accept: function () {
      Ember.ENV.LOG_DIALOG &&
        Ember.Logger.log(
          "✓ %cConfirm action%c " + this.get("name"),
          "font-weight: 900; color: #070;",
          null
        );
    ...
    keyDown: function (e) {
      ...
      if (this.get("isActive")) {
        viewsController = this.get("childViews")
          .findBy("controller")
          .get("controller");
        if (e.keyCode === 27) {
    ...
    focus: function () {
      var firstInput = this.$()
        .find("input:visible:first, button:visible:not(.close):first")
        .first();
      if (firstInput.size()) {
        firstInput.focus();
      } else {
...
define("ember-dialog/utils/highest-zindex", ["exports"], function (
  __exports__
) {
  "use strict";
  __exports__["default"] = function () {
    ...
    var max = Math.max.apply(
      null,
      $.map($(":visible:not(.highest)"), function (element) {
        if (
          $(element).css("position") === "absolute" ||
          $(element).css("position") === "relative"
        ) {
          return $(element).css("z-index") >> 0 || 1;
        }
        return 0;
      })
    );
...
define("ember-dialog/templates/components/dialog", ["exports"], function (
  __exports__
) {
  "use strict";
  __exports__["default"] = Ember.Handlebars.compile(
    '<div {{bind-attr class=":dialog-dialog :component-dialog-dialog className substrate"}}>\n    <div class="dialog-content" tabindex="-1">\n        {{view body}}\n    </div>\n</div>'
  );
});
define("ember-dialog/templates/dialogs/alert", ["exports"], function (
  __exports__
) {
  "use strict";
  __exports__["default"] = Ember.Handlebars.compile(
...
define("ember-dialog/templates/dialogs/confirm", ["exports"], function (
  __exports__
) {
  "use strict";
  __exports__["default"] = Ember.Handlebars.compile(
...
define("ember-dialog/templates/dialogs/custom", ["exports"], function (
  __exports__
) {
  "use strict";
  __exports__["default"] = Ember.Handlebars.compile("{{yield}}");
});
define("ember-dialog/templates/dialogs/notice", ["exports"], function (
  __exports__
) {
  "use strict";
  __exports__["default"] = Ember.Handlebars.compile(
...
Ember.TEMPLATES["dialogs/alert"] = Ember.Handlebars.compile(
  '<div class="dialog-header">    <button type="button" class="close" {{action dialog.acceptHandlerName dialog}}>×</button>    <div class="dialog-title">{{ dialog.title }}</div></div><div class="dialog-body">    {{yield}}</div><div class="dialog-footer">    <button class="btn btn-default" type="button" {{action dialog.acceptHandlerName dialog}}>{{dialog.acceptLabel}}</button></div>'
);
Ember.TEMPLATES["dialogs/confirm"] = Ember.Handlebars.compile(
  '<div class="dialog-header">    <button type="button" class="close" {{action dialog.declineHandlerName dialog}}>×</button>    <div class="dialog-title">{{ dialog.title }}</div></div><div class="dialog-body">    {{yield}}</div><div class="dialog-footer">    <button {{bind-attr class=":btn dialog.acceptClass"}} type="button" {{action dialog.acceptHandlerName dialog}}>{{dialog.acceptLabel}}</button>    <button {{bind-attr class=":btn dialog.declineClass"}} type="button" {{action dialog.declineHandlerName dialog}}>{{dialog.declineLabel}}</button></div>'
);
Ember.TEMPLATES["dialogs/custom"] = Ember.Handlebars.compile("{{yield}}");
Ember.TEMPLATES["dialogs/notice"] = Ember.Handlebars.compile(
  '<div class="dialog-body">    {{yield}}</div>'
);
Ember.TEMPLATES["components/dialog"] = Ember.Handlebars.compile(
  '<div {{bind-attr class=":dialog-dialog :component-dialog-dialog className substrate"}}>    <div class="dialog-content" tabindex="-1">        {{view body}}    </div></div>'
);
Ember.onLoad("Ember.Application", function (Application) {
  var initializer = require("ember-dialog/initializers/dialog-manager")[
    "default"
  ];
