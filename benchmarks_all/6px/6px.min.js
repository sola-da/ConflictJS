!(function () {
  ...
  var Promise = (this.Promise = function () {
    (this.callbacks = []), (this.state = null), (this.data = null);
  });
  (Promise.prototype.fire = function (type, data) {
    "function" != typeof type && (this.state = "success" == type ? !0 : !1),
      this.callbacks
        .filter(function (cb) {
          return "function" == typeof type
            ? cb.fn == type && !cb.fired
            : cb.type == type && !cb.fired;
        })
        .forEach(function (cb) {
          cb.fire(data);
        });
    ...
    (Promise.prototype.then = function (good, bad) {
      this.callbacks.push(new Promise.Callback(good, "success")),
        bad && this.callbacks.push(new Promise.Callback(bad, "fail")),
        null !== this.state &&
          (this.state ? this.fire(good, this.data) : this.fire(bad, this.data));
    }),
    (Promise.Callback = function (fn, type) {
      (this.fired = !1), (this.fn = fn), (this.type = type);
    }),
    (Promise.Callback.prototype.fire = function (data) {
      this.fn(data), (this.fired = !0);
    });
  var Defer = (this.Defer = function () {
    this.promise = new Promise();
  });
  (Defer.prototype.resolve = function (data) {
    (this.promise.data = data), this.promise.fire("success", data);
  }),
  ...
  (Output.prototype.tag = function (name) {
    return (this.tagName = name), this;
  }),
    (Output.prototype.resize = function (size) {
      return (
        (this.data.resize = px.mergeObject(this.data.resize || {}, size)), this
      );
    }),
    (Output.prototype.filter = function (type, value) {
      return "object" != typeof type || type instanceof Array
        ? ((this.filters[type] = value), (this.hasFilters = !0), this)
        : ((this.filters = type), (this.hasFilters = !0), this);
    }),
    (Output.prototype.url = function (location) {
      return (this.urlLocation = location), this;
    }),
    ...
    (Output.prototype.crop = function (position) {
      return (
        (this.data.crop = px.mergeObject(this.data.crop || {}, position)), this
      );
    }),
    (Output.prototype.layer = function (refName, options) {
      var action = { method: "layer", options: { ref: refName } };
      return (
        !options ||
    ...
    (Output.prototype.export = function () {
      this.hasFilters &&
        ...
        Object.keys(this.data).forEach(function (index) {
          ...
          this.actions.push({ method: index, options: val });
        }, this);
      var output = {
        ref: this.refs,
        type: this.type,
  ...
  (_6px.prototype.load = function (name, path) {
    ...
    (_6px.prototype.getInfo = function (fn) {
      ...
      return (
        ...
        this.output(refs).tag("info"),
        this.save().then(
          function (res) {
            var r = new Result.Info(res);
            d.resolve(r), fn && fn(null, r);
          },
          function (err) {
            d.reject(err), fn && fn(err);
          }
        ),
        d.promise
      );
    ...
    (_6px.prototype.upload = function (fn) {
      ...
      return (
        ...
        this.output(refs).url("6px").tag("info"),
        this.save().then(
          function (res) {
            var r = new Result.Info(res);
            d.resolve(r), fn && fn(null, r);
    ...
    (_6px.prototype.save = function (options, fn) {
      ...
      var inputs = {},
        inputTotal = Object.keys(this.images),
        inputTotalLen = inputTotal.length,
          ...
          _this.callback && (json.callback = { url: _this.callback }),
            px.sendToServer(
              "post",
              "/users/:userId/jobs",
              json,
              function (res) {
                px.once("job.done." + res.id, function () {
                  px.get(res.id, function (job) {
                    var r = new Result(job.processed);
                    d.resolve(r), fn && fn.call(_this, null, r);
                  });
                });
              },
              function () {
                d.reject("Error sending to server"),
                  fn && fn.call(_this, "Error sending to server");
              }
            );
        };
      return (
        inputTotal.forEach(function (index) {
          px.parseInput(this.images[index], function (data) {
            (inputs[index] = data), --inputTotalLen || done();
          });
        }, this),
        d.promise
      );
  ...
  var px = function (input) {
    if (!px.userData) throw "6px: You must call init!";
    return new _6px(input);
  };
  (px.init = function (data) {
    var d = new Defer();
    if (px.userData) throw "6px: Init must only be called once!";
    if (
      ((px.debug = !!data.debug || !1),
      (px.dryRun = !!data.dryRun || !1),
      !data.apiKey)
    )
      throw "6px: apiKey is required!";
    if (!data.userId) throw "6px: userId is required!";
    px.userData = data;
    var success = function (res) {
        px.openSocket(d),
          px.log(res),
          px.on("job.update", function (e, jobId, status) {
            "complete" == status && px.trigger("job.done." + jobId);
          });
      },
      failed = function (res) {
        px.log("failed:", res), px.trigger("error", "");
      };
    return (
    ...
    (px.openSocket = function (d) {
      ...
      (socket.onopen = function () {
        ...
        setInterval(function () {
          socket.send(JSON.stringify({ ping: !0 }));
        }, 3e4),
    ...
    (px.handleIncoming = function (msg, d) {
      ...
      data.auth && data.auth === !0 && (px.trigger("connection"), d.resolve()),
        data.job_id &&
          data.status &&
    ...
    (px.once = function (name, fn) {
      var listener = function (e) {
        ...
        e.detail &&
          ...
          fn.apply(null, args),
          window.removeEventListener(name, listener, !1);
      };
      window.addEventListener(name, listener, !1);
    }),
    (px.trigger = function (name) {
      var options = Array.prototype.slice.call(arguments, 1);
      "error" == name && px.log(options[0], !0),
    ...
    (px.dropZone = function (input, options) {
      ...
      if (
        ((elm =
          "string" == typeof input ? document.querySelector(input) : input),
        !elm)
      )
        return px.trigger("error", "Element is not defined"), !1;
      var wrapCallbacks = function (e, cb) {
          return e.preventDefault(), cb && cb(e), !1;
        },
        dragOver = function (e) {
    ...
    (px.parseInput = function (input, fn) {
      if ("string" == typeof input) return void fn.call(null, input);
      if (input instanceof Image) return void fn.call(null, input.src);
      if (1 === input.nodeType && "img" == input.tagName.toLowerCase())
        return void fn.call(null, input.src);
    ...
    (px.sendToServer = function (method, path, json, success, failed) {
      ...
      path = path.replace(":userId", user.userId);
      var url =
          "https://api.6px.io/v1" +
          path +
          (/\?/.test(url) ? "&" : "?") +
      ...
      (xhr.onreadystatechange = function () {
        return xhr.readyState < 4
          ? void 0
          : 200 !== xhr.status
          ? failed.call(window, JSON.parse(xhr.responseText))
          : void (
              4 === xhr.readyState &&
              success.call(null, JSON.parse(xhr.responseText))
            );
      }),
        xhr.open(method.toUpperCase(), url, !0),
        json
    ...
    (px.log = function (msg, err) {
      px.debug &&
        console &&
        console.log &&
        (err ? console.error("6px:", msg) : console.log("6px:", msg));
    }),
