!(function (c) {
  ...
  a.prototype = {
    ...
    createModal: function (f, e) {
      c(f).modal(c.extend({ manager: this }, e));
    },
      ...
      f.$element.on(
        ...
        b(function (h) {
          var g = function () {
            ...
            e.$element
              .toggleClass("modal-open", e.hasOpenModal())
              .toggleClass(
                "page-overflow",
        ...
        b(function (g) {
          ...
          if (!f.$element.parent().length) {
            ...
            if (f.$backdrop) {
              ...
              c.support.transition && f.$element.hasClass("fade")
                ? f.$backdrop.one(c.support.transition.end, function () {
                    f.destroy();
                  })
                : f.destroy();
            } else {
    ...
    createContainer: function (e) {
      var f;
      f = c('<div class="modal-scrollable">')
        .css("z-index", d("modal", this.getOpenModals().length))
    ...
    backdrop: function (h, j) {
      var f = h.$element.hasClass("fade") ? "fade" : "",
        i =
          h.options.backdrop && this.backdropCount < this.options.backdropLimit;
      if (h.isShown && i) {
        ...
        if (!h.isShown && h.$backdrop) {
          ...
          this.backdropCount -= 1;
          var g = this;
          c.support.transition && h.$element.hasClass("fade")
            ? h.$backdrop.one(c.support.transition.end, function () {
                g.removeBackdrop(h);
              })
            : g.removeBackdrop(h);
        } else {
    ...
    loading: function (h) {
      ...
      if (!this.isLoading) {
        ...
        this.$backdropHandle
          .css("z-index", d("backdrop", e.length + 1))
          .addClass("in");
        var g = c(this.options.spinner)
          .css("z-index", d("modal", e.length + 1))
          .appendTo(this.$element)
          .addClass("in");
        this.$spinner = c(this.createContainer())
          .append(g)
          .on("click.modalmanager", c.proxy(this.loading, this));
        this.isLoading = true;
        c.support.transition
          ? this.$backdropHandle.one(c.support.transition.end, h)
          : h();
      } else {
        if (this.isLoading && this.$backdropHandle) {
          ...
          var f = this;
          c.support.transition
            ? this.$backdropHandle.one(c.support.transition.end, function () {
                f.removeLoading();
              })
            : f.removeLoading();
        } else {
