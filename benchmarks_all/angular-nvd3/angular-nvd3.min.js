          ...
          link: function (scope, element, attrs) {
                  ...
                  "_" === key[0] ||
                    ("dispatch" === key
                      ? ((void 0 !== options[key] && null !== options[key]) ||
                          (scope._config.extended && (options[key] = {})),
                      ...
                      ? options[key] && chart[key](options[key])
                      : -1 ===
                          [
                            "axis",
                            "clearHighlights",
                        ...
                        (void 0 === options[key] || null === options[key]
                          ? scope._config.extended && (options[key] = value())
            ...
            function configureWrapper(name) {
              ...
              var wrapElement = angular
                ...
                .removeAttr("style")
                .css(_.css);
              _.html || wrapElement.text(_.text),
                _.enable &&
                  ("title" === name
                    ? element.prepend(wrapElement)
                    : "subtitle" === name
                    ? angular
                        .element(element[0].querySelector(".title"))
                        .after(wrapElement)
                    : "caption" === name && element.append(wrapElement));
            }
            function configureStyles() {
              ...
              scope._config.extended && (scope.options.styles = _),
            ...
            function dataWatchFn(newData, oldData) {
              newData !== oldData &&
                (scope._config.disabled ||
                  (scope._config.refreshDataOnly
                    ? scope.api.update()
                    : scope.api.refresh()));
            }
            var defaultConfig = {
            ...
            (scope.isReady = !1),
              ...
              (scope.api = {
                ...
                updateWithOptions: function (options) {
                  scope.api.clearElement(),
                    angular.isDefined(options) !== !1 &&
                      scope._config.visible &&
                      ((scope.chart = nv.models[options.chart.type]()),
                      (scope.chart.id = Math.random()
                        .toString(36)
                        .substr(2, 15)),
                      angular.forEach(scope.chart, function (value, key) {
                        "_" === key[0] ||
                          ...
                          ("dispatch" === key
                            ? ((void 0 !== options.chart[key] &&
                                null !== options.chart[key]) ||
                            ...
                            : ("focusHeight" === key &&
                              ...
                              ((("xTickFormat" !== key &&
                                ...
                                "lineWithFocusChart" !== options.chart.type) &&
                                (("tooltips" === key &&
                                  "boxPlotChart" === options.chart.type) ||
                                  ((("tooltipXContent" !== key &&
                                    "tooltipYContent" !== key) ||
                      ...
                      "sunburstChart" === options.chart.type
                        ? scope.api.updateWithData(angular.copy(scope.data))
                        : scope.api.updateWithData(scope.data),
                      (options.title || scope._config.extended) &&
                        configureWrapper("title"),
                      (options.subtitle || scope._config.extended) &&
                        configureWrapper("subtitle"),
                      (options.caption || scope._config.extended) &&
                        configureWrapper("caption"),
                      (options.styles || scope._config.extended) &&
                        configureStyles(),
                      nv.addGraph(function () {
                        return scope.chart
                          ? (scope.chart.resizeHandler &&
                ...
                updateWithData: function (data) {
                  if (data) {
                    ...
                    (scope.svg = d3.select(element[0]).append("svg")),
                      (h = scope.options.chart.height) &&
                        (isNaN(+h) || (h += "px"),
                        scope.svg.attr("height", h).style({ height: h })),
                      (w = scope.options.chart.width)
                        ? (isNaN(+w) || (w += "px"),
                ...
                clearElement: function () {
                  if (
                    ...
                    scope.chart &&
                      scope.chart.tooltip &&
                      scope.chart.tooltip.id &&
                      d3.select("#" + scope.chart.tooltip.id()).remove(),
                    nv.graphs && scope.chart)
                  )
                    for (var i = nv.graphs.length - 1; i >= 0; i--)
                      nv.graphs[i] &&
                        nv.graphs[i].id === scope.chart.id &&
              ...
              scope._config.deepWatchOptions &&
                scope.$watch(
                  "options",
                  nvd3Utils.debounce(
                    function (newOptions) {
                    ...
                    scope._config.debounce,
                    scope._config.debounceImmediate
                  ),
                  !0
                ),
              scope._config.deepWatchData &&
                (1 === scope._config.deepWatchDataDepth
                  ? scope.$watchCollection("data", dataWatchFn)
                  : scope.$watch(
                      "data",
                      dataWatchFn,
                      2 === scope._config.deepWatchDataDepth
                    )),
              scope.$watch(
                "config",
      ...
      return {
        ...
        zoom: function (scope, options) {
          var zoom = options.chart.zoom,
            enabled =
              "undefined" == typeof zoom.enabled || null === zoom.enabled
                ? !0
                : zoom.enabled;
          if (enabled) {
            var fixDomain,
            ...
            useNiceScale && (xScale.nice(), yScale.nice()),
              (fixDomain = function (domain, boundary) {
                return (
                  (domain[0] = Math.min(
                    Math.max(domain[0], boundary[0]),
                    boundary[1] - boundary[1] / scaleExtent[1]
                  )),
                  (domain[1] = Math.max(
                    boundary[0] + boundary[1] / scaleExtent[1],
                    Math.min(domain[1], boundary[1])
                  )),
                  domain
                );
              }),
              (zoomed = function () {
                if (void 0 !== zoom.zoomed) {
                  ...
                  horizontalOff ||
                    xDomain(
                      useFixedDomain
                        ? fixDomain(xScale.domain(), x_boundary)
                        : xScale.domain()
                    ),
                    verticalOff ||
                      yDomain(
                        useFixedDomain
                          ? fixDomain(yScale.domain(), y_boundary)
                          : yScale.domain()
                      );
                scope.chart.update();
              }),
              (unzoomed = function () {
                if (void 0 !== zoom.unzoomed) {
                  ...
                  horizontalOff || xDomain(x_boundary),
                    verticalOff || yDomain(y_boundary);
