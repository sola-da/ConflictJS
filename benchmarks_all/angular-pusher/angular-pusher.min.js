...
angular
  ...
  .provider("PusherService", function () {
    ...
    (this.setPusherUrl = function (url) {
      return url && (scriptUrl = url), this;
    }),
      (this.setOptions = function (options) {
        return (initOptions = options || initOptions), this;
      }),
      (this.setToken = function (token) {
        return (apiKey = token || apiKey), this;
      }),
      (this.$get = [
        "$document",
        "$timeout",
        ...
        function (
          ...
          var pusher,
            deferred = $q.defer(),
            es = PusherEventsService.connection,
          ...
          return createScript($document[0], onScriptLoad), deferred.promise;
        },
      ]);
  })
  .factory("Pusher", [
    ...
    function ($rootScope, $q, PusherService, PusherEventsService) {
      return {
            ...
            PusherService.then(function (pusher) {
              ...
              channel
                ...
                .bind(PusherEventsService.channel.error, function (status) {
                  $rootScope.$broadcast(
                    PusherEventsService.channel.error,
                    status
                  );
                }),
                -1 != channelName.indexOf("presence-") &&
                  channel
                    .bind(
                      PusherEventsService.presence.success,
                      function (members) {
                        var dataObj = { count: members.count, list: [] };
                    ...
                    .bind(
                      PusherEventsService.presence.memberAdded,
                      function (member) {
                        $rootScope.$broadcast(
                          PusherEventsService.presence.memberAdded,
                          member
                    ...
                    .bind(
                      PusherEventsService.presence.memberRemoved,
                      function (member) {
                        $rootScope.$broadcast(
                          PusherEventsService.presence.memberAdded,
                          member
        ...
        unsubscribe: function (channelName) {
          PusherService.then(function (pusher) {
            pusher.channel(channelName) && pusher.unsubscribe(channelName);
          });
        },
        trigger: function (channelName, eventName, obj) {
          return -1 == eventName.indexOf("client-")
            ? "Event name requires 'client-' prefix."
            : void PusherService.then(function (pusher) {
        ...
        members: function (channelName) {
          return -1 == channelName.indexOf("presence-")
            ? "Requires presence channel."
            : void PusherService.then(function (pusher) {
                var presenceChannel = pusher.channel(channelName),
                  arr = [];
                return (
                  presenceChannel.members.each(function (member) {
                    arr.push({ id: member.id, info: member.info });
                  }),
                  arr
                );
              });
        ...
        membersCount: function (channelName) {
          return -1 == channelName.indexOf("presence-")
            ? "Requires presence channel."
            : void PusherService.then(function (pusher) {
                var presenceChannel = pusher.channel(channelName);
