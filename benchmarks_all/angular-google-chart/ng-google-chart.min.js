  ...
  function formatManagerFactory() {
    function FormatManager($google) {
          ...
          if (tFormats.hasOwnProperty(formatType)) {
            for (i = 0; i < self.iFormats[formatType].length; i++) {
              if (
                ...
                self.iFormats[formatType][i].format(
                  dataTable,
                  tFormats[formatType][i].columnNum
                );
              }
          ...
          if (
            ...
            if (formatType === "color") {
              ...
              for (i = 0; i < tFormats[formatType].length; i++) {
                self.iFormats[formatType].push(
                  new FormatClass(tFormats[formatType][i])
                );
              }
            }
          }
      ...
      function instantiateColorFormatters(tFormats) {
        ...
        for (t = 0; t < tFormats[formatType].length; t++) {
          ...
          for (i = 0; i < tFormats[formatType][t].formats.length; i++) {
            ...
            if (
              typeof data.fromBgColor !== "undefined" &&
              typeof data.toBgColor !== "undefined"
            ) {
              colorFormat.addGradientRange(
                data.from,
                data.to,
                data.color,
                data.fromBgColor,
      ...
      function getFormatClass(formatType, customFormatters) {
        var className =
          formatType.charAt(0).toUpperCase() +
          formatType.slice(1).toLowerCase() +
          "Format";
        if ($google.visualization.hasOwnProperty(className)) {
          return google.visualization[className];
        } else if (
          angular.isDefined(customFormatters) &&
  ...
  function onReadyDirective() {
    return {
      ...
      link: function (scope, element, attrs, googleChartController) {
        ...
        googleChartController.registerServiceListener(
          "beforeDraw",
          callback,
          this
        );
  ...
  function agcOnMouseoutDirective() {
    return {
      ...
      link: function (scope, element, attrs, googleChartController) {
        ...
        googleChartController.registerChartListener(
          "onmouseout",
          callback,
          this
        );
    ...
    return {
      ...
      link: function (scope, element, attrs, googleChartController) {
        ...
        function callback(chartWrapper, chart) {
          ...
          selectEventRetParams.selectedItem =
            selectEventRetParams.selectedItems[0];
...
(function () {
  ...
  function googleChartDirective() {
    return { restrict: "A", scope: false, controller: "GoogleChartController" };
  }
})();
(function () {
...
(function () {
  ...
  googleChartApiPromiseFactory.$inject = [
    "$rootScope",
    "$q",
...
(function () {
  ...
  GoogleChartServiceFactory.$inject = [
    "googleChartApiPromise",
    "$injector",
    "$q",
    "FormatManager",
  ];
  function GoogleChartServiceFactory(
    googleChartApiPromise,
    $injector,
    $q,
    FormatManager
    ...
    function GoogleChartService() {
      ...
      function _continueSetup() {
        ...
        if (
          _formatManager.applyFormats(
            _chartWrapper.getDataTable(),
            _formatters,
            _customFormatters
          ).requiresHtml
        ) {
      ...
      function _registerListener(
        ...
        if (
          angular.isDefined(listenerCollection) &&
          ...
          return function () {
            if (angular.isDefined(listenerWrapper.googleListenerHandle)) {
              $google.visualization.events.removeListener(
                listenerWrapper.googleListenerHandle
              );
            }
            var fnIndex =
              listenerCollection[eventName].indexOf(listenerWrapper);
      ...
      function _registerListenersWithGoogle(eventSource, listenerCollection) {
        for (var eventName in listenerCollection) {
          if (
            listenerCollection.hasOwnProperty(eventName) &&
            angular.isArray(listenerCollection[eventName])
          ) {
            for (
              var fnIterator = 0;
              fnIterator < listenerCollection[eventName].length;
              fnIterator++
            ) {
              if (
                angular.isFunction(listenerCollection[eventName][fnIterator])
              ) {
