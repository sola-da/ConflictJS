  ...
  })(
    {
      1: [
        function (require, module, exports) {
          ...
          function getRangeAxisOrder(domain, range) {
            if (!domain) return range._axisNames;
            var needsRangeAxisOrder =
                []
                  .concat(_toConsumableArray(domain.axes.values()))
                  .filter(function (axis) {
                    return axis.values.length > 1;
                  }).length > 1,
              axisOrder = domain._rangeAxisOrder || range._axisNames;
            if (needsRangeAxisOrder && !axisOrder)
              throw new Error("Range axis order missing");
            return (axisOrder =
              axisOrder || [].concat(_toConsumableArray(domain.axes.keys())));
          }
          function getRangeShapeArray(domain, range) {
            if (!domain) return range._shape;
            var axisOrder = getRangeAxisOrder(domain, range),
              shape = axisOrder.map(function (k) {
                return domain.axes.get(k).values.length;
              });
            if (range._shape) {
              var matchesDomain =
                range._shape.length === shape.length &&
                range._shape.every(function (v, i) {
                  return v === shape[i];
                });
              if (!matchesDomain)
          ...
          function loadRangesFn(cov) {
            ...
            return function (paramKeys) {
              return (
                void 0 === paramKeys && (paramKeys = _this3.parameters.keys()),
                ...
                Promise.all(
                  ...
                  for (var map = new Map(), i = 0; i < paramKeys.length; i++)
          ...
          function doLoadRange(cov, paramKey, range, domain) {
            ...
            if (
              ((globalConstraints = (0,
              _subset.normalizeIndexSubsetConstraints)(
                domain,
                globalConstraints
              )),
              "NdArray" === range.type || "Range" === range.type)
            ) {
              transformNdArrayRange(range, domain),
                cov.options.cacheRanges &&
                  ((cov._covjson.ranges[paramKey] = range),
                  cov._updateLoadStatus());
              var newrange = subsetNdArrayRangeByIndex(
                range,
                domain,
                globalConstraints
              );
          ...
          function loadTiledNdArraySubset(range, constraints) {
            for (
              var constraintsArr = range.axisNames.map(function (name) {
                ...
                fillNulls = function fillNulls(tileShape) {
                  return tileShape.map(function (v, i) {
                    return null === v ? range.shape[i] : v;
                  });
                },
                tilesetsStats = range.tileSets.map(function (ts) {
                  return getTilesetStats(
                    fillNulls(ts.tileShape),
                    constraintsArr
                  );
                }),
                idxBestTileset = indexOfBestTileset(tilesetsStats),
                tileset = range.tileSets[idxBestTileset],
                tileShape = fillNulls(tileset.tileShape),
                subsetTilesetAxes = [],
                ax = 0;
              ax < tileShape.length;
              ax++
              ...
              for (
                var _constraintsArr$ax = constraintsArr[ax],
                  start = _constraintsArr$ax.start,
                  stop = _constraintsArr$ax.stop,
                  step = _constraintsArr$ax.step,
                  tileSize = tileShape[ax],
                  tileStart = Math.floor(start / tileSize),
                  tileStop = Math.ceil(stop / tileSize),
                  tilesetAxis = [],
                  t = tileStart;
                tileStop > t;
                t++
                ...
                var mid = (t + 0.5) * tileSize,
                  iv = Math.floor((mid - start) / step),
                  ivStart = start + iv * step,
                  ivStop = start + (iv + 1) * step,
                  tileStartR = t * tileSize,
                  tileStopR = (t + 1) * tileSize;
                (ivStart >= tileStartR || ivStop >= tileStopR) &&
                  tilesetAxis.push(t);
              }
            ...
            var subsetShape = constraintsArr.map(function (_ref) {
                var start = _ref.start,
                  ...
                  step = _ref.step;
                return (
                  Math.floor((stop - start) / step) + ((stop - start) % step)
                );
              }),
              subsetSize = subsetShape.reduce(function (l, r) {
                return l * r;
              }),
              subsetNdArr = (0, _ndarray2["default"])(
                new Array(subsetSize),
                subsetShape
              ),
              urlTemplate = _urlTemplate2["default"].parse(tileset.urlTemplate),
              tiles = cartesianProduct(subsetTilesetAxes),
              promises = tiles.map(function (tile) {
                var tileUrlVars = {};
                ...
                return (0, _http.load)(url).then(function (result) {
                  ...
                  for (
                    var tileOffsets = tile.map(function (v, i) {
                      ...
                      tileAxesSubsetIndices = [],
                      _ax = 0;
                    _ax < tileShape.length;
                    _ax++
                  ) {
                    var _constraintsArr$_ax = constraintsArr[_ax],
                      start = _constraintsArr$_ax.start,
                      stop = _constraintsArr$_ax.stop,
                      step = _constraintsArr$_ax.step,
                      tileAxisSize = tileShape[_ax],
                      tileAxisOffset = tileOffsets[_ax],
                      tileAxisSubsetIndices = [],
                      startIdx = 0;
                    start > tileAxisOffset &&
                      (startIdx = start - tileAxisOffset);
                    var stopIdx = tileAxisSize;
                    tileAxisOffset + stopIdx > stop &&
                      (stopIdx = stop - tileAxisOffset);
                    for (var i = startIdx; stopIdx > i; i++) {
                  ...
                  var tileSubsetIndices = cartesianProduct(
                      tileAxesSubsetIndices
                    ),
                  ...
                  try {
                    for (
                      var _iterator2 = tileSubsetIndices[Symbol.iterator](),
                        _step2;
                      ...
                      var _tileRange$_ndarr,
                        tileInd = _step2.value,
                        val = (_tileRange$_ndarr = tileRange._ndarr).get.apply(
                          _tileRange$_ndarr,
                          _toConsumableArray(tileInd)
                        ),
                        subsetInd = tileInd.map(function (i, ax) {
                          var idx = tileOffsets[ax] + i;
                          return Math.floor(
                            (idx - constraintsArr[ax].start) /
                              constraintsArr[ax].step
                          );
                        });
                      subsetNdArr.set.apply(
                        subsetNdArr,
                        _toConsumableArray(subsetInd).concat([val])
                      );
                    }
                  } catch (err) {
            ...
            return Promise.all(promises).then(function () {
              ...
              return (
                (newrange.shape = new Map(
                  range.axisNames.map(function (v, i) {
          ...
          function getTilesetStats(tileShape, constraints) {
            for (var tileCount = 1, i = 0; i < tileShape.length; i++) {
              var _constraints$i = constraints[i],
                start = _constraints$i.start,
                stop = _constraints$i.stop,
                step = _constraints$i.step,
                tileSize = tileShape[i],
                tileStart = Math.floor(start / tileSize),
                tileStop = Math.ceil(stop / tileSize),
                nvalues = tileSize * (tileStop - tileStart);
              tileCount *= Math.ceil(nvalues / Math.max(step, tileSize));
            }
            var valueCount =
              tileCount *
              tileShape.reduce(function (l, r) {
                return l * r;
              });
            return { tileCount: tileCount, valueCount: valueCount };
          }
          function indexOfBestTileset(tilesetsStats) {
            var tileValueRatio = 1e3,
              efforts = tilesetsStats.map(function (s) {
                return s.tileCount + s.valueCount / tileValueRatio;
              }),
              minEffortIdx = efforts.reduce(function (imin, x, i, arr) {
          ...
          function subsetNdArrayRangeByIndex(range, domain, constraints) {
            var _ndarr$hi$lo,
              _ndarr$hi,
              ndarr = range._ndarr,
              axisNames = getRangeAxisOrder(domain, range),
              los = axisNames.map(function (name) {
                return constraints[name].start;
              }),
              his = axisNames.map(function (name) {
                return constraints[name].stop;
              }),
              steps = axisNames.map(function (name) {
                return constraints[name].step;
              }),
              newndarr = (_ndarr$hi$lo = (_ndarr$hi = ndarr.hi.apply(
                ndarr,
                _toConsumableArray(his)
              )).lo.apply(_ndarr$hi, _toConsumableArray(los))).step.apply(
                _ndarr$hi$lo,
          ...
          function subsetByIndexFn(cov, globalConstraints) {
            return function (constraints) {
              return cov.loadDomain().then(function (domain) {
                constraints = (0, _subset.normalizeIndexSubsetConstraints)(
                  domain,
                  constraints
                );
                var newdomain = (0, _subset.subsetByIndex)(domain, constraints),
                  newGlobalConstraints = toGlobalSubsetConstraints(
                    constraints,
                    globalConstraints
                  );
                ...
                return (
                  (newcov.loadRange = loadRangeFn(
                    newcov,
                    newGlobalConstraints
                  )),
                  (newcov.loadRanges = loadRangesFn(newcov)),
                  (newcov.subsetByIndex = subsetByIndexFn(
                    newcov,
                    newGlobalConstraints
                  )),
                  (newcov.subsetByValue = _subset2.subsetByValue.bind(
                    null,
                    newcov
                  )),
                  newcov
          ...
          function transformParameter(params, key) {
            if (!("__transformDone" in params[key])) {
              ...
              if (
                ((param.key = key),
                param.unit &&
                  "string" == typeof param.unit.symbol &&
                  (param.unit.symbol = { value: param.unit.symbol }),
                param.categoryEncoding)
              ) {
                var map = new Map(),
                ...
                try {
                  for (
                    var _iterator3 = Object.keys(param.categoryEncoding)[
                        Symbol.iterator
                      ](),
                      _step3;
                    ...
                    var category = _step3.value,
                      vals = param.categoryEncoding[category];
              ...
              param.__transformDone = !0;
            }
          }
          function transformNdArrayRange(range, domain) {
            if (!("__transformDone" in range)) {
              var values = range.values,
                targetDataType = range.dataType,
                isTyped = ArrayBuffer.isView(values),
                missingIsEncoded = "number" == typeof range.validMin,
                hasOffsetFactor = "offset" in range;
              "offset" in range && (0, _util.assert)("factor" in range);
              ...
              if (missingIsEncoded || hasOffsetFactor) {
                if (((vals = new Array(values.length)), hasOffsetFactor)) {
                  for (var i = 0; i < values.length; i++) {
                    var val = values[i];
                    missingIsEncoded && (validMin > val || val > validMax)
                      ? (vals[i] = null)
                      : missingIsEncoded || null !== val
              ...
              if (void 0 === range.actualMin) {
                var _minMax = (0, _array.minMax)(vals),
                  _minMax2 = _slicedToArray(_minMax, 2),
                  min = _minMax2[0],
                  max = _minMax2[1];
                null !== min &&
                  ((range.actualMin = min), (range.actualMax = max));
              }
              range.shape && (range._shape = range.shape),
                range.axisNames &&
                  ((range._axisNames = range.axisNames),
                  delete range.axisNames);
              var axisNames = getRangeAxisOrder(domain, range),
                shapeArr = getRangeShapeArray(domain, range),
                ndarr = (0, _ndarray2["default"])(vals, shapeArr);
              return (
                (range._ndarr = ndarr),
                ...
                (range.__transformDone = !0),
                range
              );
            }
          }
          function createRangeGetFunction(ndarr, axisOrder) {
            for (var ndargs = "", i = 0; i < axisOrder.length; i++)
              ndargs && (ndargs += ","),
                (ndargs +=
                  "'" +
                  axisOrder[i] +
            ...
            var fn = new Function(
              "ndarr",
              "return function ndarrget (obj) { return ndarr.get(" +
                ndargs +
                ") }"
            )(ndarr);
            return fn;
          ...
          function transformDomain(domain, referencing, domainType) {
            if (!("__transformDone" in domain)) {
              ...
              try {
                for (
                  var _iterator4 = Object.keys(domain.axes)[Symbol.iterator](),
                    _step4;
              ...
              try {
                for (
                  var _iterator5 = axes[Symbol.iterator](), _step5;
                  ...
                  if (
                    ((axis.key = key),
                    ("Tuple" !== axis.dataType &&
                      "Polygon" !== axis.dataType) ||
                      (axis.dataType = _util.PREFIX + axis.dataType),
                    axis.dimensions && (axis.components = axis.dimensions),
                    axis.components || (axis.components = [key]),
                    (axis.dimensions = axis.components),
                    "start" in axis && "stop" in axis && "num" in axis)
                  ) {
                    ...
                    if (1 === axis.num) {
                      if (axis.start !== axis.stop)
                        throw new Error(
                          "regular axis of length 1 must have equal start/stop values"
                        );
                      step = 0;
                    } else step = (axis.stop - axis.start) / (axis.num - 1);
                  ...
                  if (!ArrayBuffer.isView(axis.values)) {
                    if (
                      ...
                      "number" == typeof axis.values[0]
                    ) {
                      for (
                        var _arr = new Float64Array(axis.values.length),
                          _i2 = 0;
                        _i2 < axis.values.length;
                        _i2++
                      )
              ...
              referencing && (domain.referencing = referencing);
              var _iteratorNormalCompletion6 = !0,
                _didIteratorError6 = !1,
                _iteratorError6 = void 0;
              try {
                for (
                  var _iterator6 = domain.referencing[Symbol.iterator](),
                    _step6;
                  ...
                  (obj.system = obj.srs || obj.trs || obj.rs),
                    obj.dimensions && (obj.components = obj.dimensions),
                    delete obj.srs,
                    delete obj.trs,
                    delete obj.rs;
                }
          ...
          function toGlobalSubsetConstraints(localConstraints) {
            ...
            try {
              for (
                var _iterator7 =
                    Object.keys(localConstraints)[Symbol.iterator](),
                  _step7;
                ...
                var axis = _step7.value,
                  local = localConstraints[axis],
                  _ref2 = globalConstraints[axis] || {},
                  _ref2$start = _ref2.start,
                  globalStart = void 0 === _ref2$start ? 0 : _ref2$start,
                  _ref2$step = _ref2.step,
                  globalStep = void 0 === _ref2$step ? 1 : _ref2$step;
                res[axis] = {
          ...
          var _ndarray = require("ndarray"),
            ...
            _subset = require("covutils/lib/domain/subset.js"),
            _subset2 = require("covutils/lib/coverage/subset.js"),
            _array = require("covutils/lib/array.js"),
            ...
            Coverage = (function () {
              function Coverage(covjson, options) {
                ...
                try {
                  for (
                    var _iterator = Object.keys(covjson.parameters)[
                        Symbol.iterator
                      ](),
                      _step;
                ...
                (domainProfile =
                  "string" == typeof this._covjson.domain
                    ? this._covjson.domainProfile || this._covjson.domainType
                    : this._covjson.domain.profile ||
                      this._covjson.domain.domainType ||
                      this._covjson.domainProfile ||
                      this._covjson.domainType),
                  domainProfile &&
                    ("http" !== domainProfile.substr(0, 4) &&
                ...
                _createClass(Coverage, [
                  {
                    ...
                    value: function _updateLoadStatus() {
                      var _this = this,
                        isLoaded = function isLoaded(prop) {
                          return (
                            "object" ===
                        ...
                        domainLoaded = isLoaded(this._covjson.domain),
                        rangesLoaded = Object.keys(this._covjson.ranges).every(
                          function (key) {
                  ...
                  },
                  {
                    ...
                    value: function loadDomain() {
                      var _this2 = this,
                        domainOrUrl = this._covjson.domain;
                      if (this._domainPromise) return this._domainPromise;
                      var promise = void 0;
                      if (
                        "object" ===
                        ("undefined" == typeof domainOrUrl
                          ? "undefined"
                          : _typeof(domainOrUrl))
                      ) {
                      ...
                      return (this._domainPromise = promise), promise;
                    },
                  },
                  {
                    key: "loadRange",
        ...
        },
        {
          ...
          "./http": 4,
          "./util.js": 7,
          "covutils/lib/array.js": 9,
          "covutils/lib/coverage/subset.js": 12,
          "covutils/lib/domain/subset.js": 14,
          ndarray: 15,
          "url-template": 18,
        },
      ],
      2: [
        function (require, module, exports) {
          ...
          var _createClass = (function () {
            ...
            _constants = require("./constants.js"),
            _Coverage = require("./Coverage.js"),
            _Coverage2 = _interopRequireDefault(_Coverage),
            _util = require("./util.js"),
            _create = require("covutils/lib/collection/create.js"),
              ...
              function CoverageCollection(covjson) {
                ...
                profile &&
                  ("http" !== profile.substr(0, 4) &&
                    ((0, _util.endsWith)(
                      profile,
                      _constants.COVERAGECOLLECTION
                    ) &&
                      (domainType = profile.substr(
                        0,
                        profile.length - _constants.COVERAGECOLLECTION.length
                      )),
                    (profile = _util.PREFIX + profile)),
                  this.profiles.push(profile)),
                  domainType || (domainType = covjson.domainType),
                  domainType &&
                    "http" !== domainType.substr(0, 4) &&
                  ...
                  !profile &&
                    ...
                    this.profiles.push(profile));
                var covs = [],
                  rootParams = covjson.parameters ? covjson.parameters : {},
                  _iteratorNormalCompletion = !0,
                  _didIteratorError = !1,
                  _iteratorError = void 0;
                try {
                  for (
                    var _iterator = Object.keys(rootParams)[Symbol.iterator](),
                      _step;
                    !(_iteratorNormalCompletion = (_step = _iterator.next())
                      .done);
                    _iteratorNormalCompletion = !0
                ...
                try {
                  for (
                    var _iterator2 = covjson.coverages[Symbol.iterator](),
                      _step2;
                    ...
                    try {
                      for (
                        var _iterator4 =
                            Object.keys(rootParams)[Symbol.iterator](),
                          _step4;
                    ...
                    covjson["@context"] &&
                      ...
                      covs.push(
                        new _Coverage2["default"](coverage, covOptions)
                      );
                  }
                } catch (err) {
                  (_didIteratorError2 = !0), (_iteratorError2 = err);
                ...
                if (((this.coverages = covs), covjson.parameters)) {
                  this.parameters = new Map();
                  ...
                  try {
                    for (
                      var _iterator3 = Object.keys(covjson.parameters)[
                          Symbol.iterator
                        ](),
                        _step3;
                      ...
                      (0, _Coverage.transformParameter)(
                        covjson.parameters,
                        key
                      ),
                        this.parameters.set(key, covjson.parameters[key]);
        ...
        },
        {
          ...
          "./constants.js": 3,
          "./util.js": 7,
          "covutils/lib/collection/create.js": 10,
        },
      ],
      3: [
        function (require, module, exports) {
          ...
          var LINKRELPREFIX = (exports.LINKRELPREFIX =
            "http://www.iana.org/assignments/relation/");
        },
        { "covutils/lib/constants.js": 11 },
      ],
      4: [
        function (require, module, exports) {
          "use strict";
            ...
            var options =
              ...
              responseType =
                arguments.length <= 2 || void 0 === arguments[2]
                  ? "arraybuffer"
                  : arguments[2];
            if (-1 === ["arraybuffer", "text"].indexOf(responseType))
              throw new Error();
            var headers = options.headers || {};
            return new Promise(function (resolve, reject) {
              var req = new XMLHttpRequest();
              req.open("GET", url), (req.responseType = responseType);
              var accept = (0, _httpCommon.getAcceptHeader)(options.eagerload);
              if ((req.setRequestHeader("Accept", accept), headers)) {
                var _iteratorNormalCompletion = !0,
                  _didIteratorError = !1,
                  _iteratorError = void 0;
                try {
                  for (
                    var _iterator = Object.keys(headers)[Symbol.iterator](),
                      _step;
                    !(_iteratorNormalCompletion = (_step = _iterator.next())
                      .done);
                    _iteratorNormalCompletion = !0
              ...
              req.addEventListener("load", function () {
                try {
                  ...
                  (0, _httpCommon.matchesMediaTypes)(type, [
                    _httpCommon.MEDIATYPE.OCTETSTREAM,
                    _httpCommon.MEDIATYPE.TEXT,
                  ]) &&
                    ((0, _util.endsWith)(url, _httpCommon.EXT.COVJSON)
                      ? (type = _httpCommon.MEDIATYPE.COVJSON)
                      : (0, _util.endsWith)(url, _httpCommon.EXT.COVCBOR) &&
                        (type = _httpCommon.MEDIATYPE.COVCBOR));
                  var data = void 0;
                  if (
                    (0, _httpCommon.matchesMediaTypes)(
                      type,
                      _httpCommon.MEDIATYPE.COVCBOR
                    )
                  ) {
                    ...
                    if (
                      !(0, _httpCommon.matchesMediaTypes)(type, [
                        _httpCommon.MEDIATYPE.COVJSON,
                        _httpCommon.MEDIATYPE.JSONLD,
                        _httpCommon.MEDIATYPE.JSON,
                      ])
                    )
                      return void reject(
                        new Error("Unsupported media type: " + type)
                      );
                    if ("arraybuffer" === responseType) {
                      if (!window.TextDecoder)
                        return void reject({ responseType: "text" });
                      var _t = new Date();
                      (data = JSON.parse(
                        new TextDecoder().decode(new DataView(req.response))
                      )),
                        console.log(
                          "JSON decoding: " + (new Date() - _t) + "ms"
                        );
                    } else {
                      var _t2 = new Date();
                      (data = JSON.parse(req.response)),
                        console.log(
                          "JSON decoding (slow path): " +
                            (new Date() - _t2) +
                            "ms"
                        );
                    }
                  ...
                  resolve({ data: data, headers: responseHeaders });
                } catch (e) {
                  reject(e);
                }
              }),
              ...
              if (e.responseType) return load(url, headers, e.responseType);
              throw e;
            });
          }
          ...
          var _cborJs = require("cbor-js"),
            _cborJs2 = _interopRequireDefault(_cborJs),
            _util = require("./util.js"),
            _httpCommon = require("./http-common.js");
        },
        { "./http-common.js": 5, "./util.js": 7, "cbor-js": 8 },
      ],
      5: [
        function (require, module, exports) {
          "use strict";
          function getAcceptHeader(standalone) {
            var covjsonProfile = standalone
                ? '; profile="' + COVJSON_PROFILE_STANDALONE + '"'
                : "",
              accept =
                MEDIATYPE.COVCBOR +
                "; q=1.0, " +
          ...
          function matchesMediaTypes(mediaType, matchingMediaTypes) {
            return (
              Array.isArray(matchingMediaTypes) ||
                (matchingMediaTypes = [matchingMediaTypes]),
              matchingMediaTypes.some(function (t) {
                return 0 === mediaType.indexOf(t);
              })
            );
          }
          Object.defineProperty(exports, "__esModule", { value: !0 }),
          ...
          var MEDIATYPE = (exports.MEDIATYPE = {
            ...
            COVJSON_PROFILE_STANDALONE = (exports.COVJSON_PROFILE_STANDALONE =
              "http://coveragejson.org/profiles/standalone"),
            EXT = (exports.EXT = { COVJSON: ".covjson", COVCBOR: ".covcbor" });
        },
        {},
      ],
      6: [
        function (require, module, exports) {
          ...
          function read(input) {
            ...
            return "object" ===
              ...
              : load(input, options).then(function (_ref) {
                  var data = _ref.data,
          ...
          function transformCovJSON(obj, headers) {
            if (
              ...
              throw new Error(
                "CoverageJSON document must be of Coverage, CoverageCollection, or Domain type"
              );
            var result = void 0;
            return (
              obj.type === _constants.DOMAIN
                ? ((0, _Coverage.transformDomain)(obj), (result = obj))
                : (result =
                    obj.type === _constants.COVERAGE
                      ? new _Coverage2["default"](obj)
                      : new _CoverageCollection2["default"](obj)),
              (obj.type !== _constants.COVERAGE &&
                obj.type !== _constants.COVERAGECOLLECTION) ||
          ...
          function addLinkRelations(cov, headers) {
            if (headers && headers.link) {
              ...
              try {
                for (
                  var _iterator = headers.link.split(",")[Symbol.iterator](),
                    _step;
                  !(_iteratorNormalCompletion = (_step = _iterator.next())
                    .done);
                  _iteratorNormalCompletion = !0
                  ...
                  var parts = link.split(";"),
                    url = parts[0].substr(1, parts[0].length - 2),
                    _iteratorNormalCompletion2 = !0,
                    _didIteratorError2 = !1,
                    _iteratorError2 = void 0;
                  try {
                    for (
                      var _iterator2 = parts.slice(1)[Symbol.iterator](),
                        _step2;
                      !(_iteratorNormalCompletion2 = (_step2 =
                        _iterator2.next()).done);
                      _iteratorNormalCompletion2 = !0
                      ...
                      var param = _step2.value,
                        relStart = param.indexOf("rel=");
                      if (-1 !== relStart) {
                        var rel = param.substring(
                          relStart + 5,
                          param.length - 1
                        );
                        rel.startsWith("http://") ||
          ...
          function checkValidCovJSON(obj) {
            (0, _util.assert)("type" in obj, '"type" missing'),
              obj.type === _constants.COVERAGE
                ...
                : obj.type === _constants.COVERAGECOLLECTION &&
                  (0, _util.assert)(
                    Array.isArray(obj.coverages),
                    '"coverages" must be an array'
                  );
          }
          Object.defineProperty(exports, "__esModule", { value: !0 });
          var _typeof =
          ...
          (exports.load = load), (exports.read = read);
          var _Coverage = require("./Coverage.js"),
            _Coverage2 = _interopRequireDefault(_Coverage),
            _CoverageCollection = require("./CoverageCollection.js"),
            _CoverageCollection2 = _interopRequireDefault(_CoverageCollection),
            _util = require("./util.js"),
            _constants = require("./constants.js"),
            _http = require("./http"),
            http = _interopRequireWildcard(_http);
        },
        {
          "./Coverage.js": 1,
          "./CoverageCollection.js": 2,
          "./constants.js": 3,
          "./http": 4,
          "./util.js": 7,
        },
      ...
      7: [
        function (require, module, exports) {
          ...
          function assert(condition, message) {
            if (!condition)
              throw (
                ((message = message || "Assertion failed"), new Error(message))
              );
          }
          ...
          function endsWith(subject, search) {
            var position = subject.length - search.length,
              lastIndex = subject.indexOf(search, position);
        ...
        function (require, module, exports) {
          !(function (global, undefined) {
            ...
            function encode(value) {
              function ensureSpace(length) {
                ...
                if (newByteLength !== data.byteLength) {
                  ...
                  for (
                    var uint32count = (offset + 3) >> 2, i = 0;
                    uint32count > i;
                    ++i
                  )
                    dataView.setUint32(4 * i, oldDataView.getUint32(4 * i));
                }
              ...
              function writeUint8Array(value) {
                for (
                  var dataView = ensureSpace(value.length), i = 0;
                  i < value.length;
                  ++i
                )
              ...
              function writeTypeAndLength(type, length) {
                24 > length
                  ...
                  : 256 > length
                  ? (writeUint8((type << 5) | 24), writeUint8(length))
                  : 65536 > length
                  ? (writeUint8((type << 5) | 25), writeUint16(length))
                  : 4294967296 > length
                  ? (writeUint8((type << 5) | 26), writeUint32(length))
                  : (writeUint8((type << 5) | 27), writeUint64(length));
              }
              function encodeItem(value) {
                var i;
                if (value === !1) return writeUint8(244);
                if (value === !0) return writeUint8(245);
                if (null === value) return writeUint8(246);
                if (value === undefined) return writeUint8(247);
                switch (typeof value) {
                  case "number":
                    if (Math.floor(value) === value) {
                      if (value >= 0 && POW_2_53 >= value)
                        return writeTypeAndLength(0, value);
                      if (value >= -POW_2_53 && 0 > value)
                    ...
                    for (i = 0; i < value.length; ++i) {
                      ...
                      128 > charCode
                        ...
                        ? (utf8data.push(224 | (charCode >> 12)),
                          ...
                          utf8data.push(128 | (63 & charCode)))
                        : ((charCode = (1023 & charCode) << 10),
                          (charCode |= 1023 & value.charCodeAt(++i)),
                          (charCode += 65536),
              ...
              var data = new ArrayBuffer(256),
                ...
                lastLength,
                offset = 0;
              if ((encodeItem(value), "slice" in data))
            ...
            function decode(data, tagger, simpleValue) {
              ...
              function readFloat16() {
                var tempArrayBuffer = new ArrayBuffer(4),
                  ...
                  value = readUint16(),
                  sign = 32768 & value,
                  exponent = 31744 & value,
                  fraction = 1023 & value;
                if (31744 === exponent) exponent = 261120;
                else if (0 !== exponent) exponent += 114688;
                else if (0 !== fraction) return fraction * POW_2_24;
                return (
              ...
              function readBreak() {
                return 255 !== dataView.getUint8(offset)
                  ? !1
                  : ((offset += 1), !0);
              }
              function readLength(additionalInformation) {
                if (24 > additionalInformation) return additionalInformation;
              ...
              function readIndefiniteStringLength(majorType) {
                ...
                if (0 > length || initialByte >> 5 !== majorType)
              ...
              function decodeItem() {
                var initialByte = readUint8(),
                  ...
                  additionalInformation = 31 & initialByte,
                  i,
                  length;
                if (7 === majorType)
                ...
                if (
                  ...
                  0 > length && (2 > majorType || majorType > 6))
                )
                ...
                switch (majorType) {
                  ...
                  case 2:
                    if (0 > length) {
                      for (
                        var elements = [], fullArrayLength = 0;
                  ...
                  case 4:
                    var retArray;
                    if (0 > length)
                      for (retArray = []; !readBreak(); )
                        retArray.push(decodeItem());
                    else
                      for (retArray = new Array(length), i = 0; length > i; ++i)
              ...
              var dataView = new DataView(data),
                offset = 0;
              "function" != typeof tagger &&
                (tagger = function (value) {
                  return value;
                }),
                "function" != typeof simpleValue &&
            ...
            var POW_2_24 = Math.pow(2, -24),
              ...
              obj = { encode: encode, decode: decode };
            "function" == typeof define && define.amd
              ? define("cbor/cbor", obj)
              : "undefined" != typeof module && module.exports
              ? (module.exports = obj)
              : global.CBOR || (global.CBOR = obj);
          })(this);
        },
        {},
      ],
      9: [
        function (require, module, exports) {
          "use strict";
          function minMax(arr) {
            for (var len = arr.length, min = 1 / 0, max = -(1 / 0); len--; ) {
              var el = arr[len];
              null == el || (min > el ? (min = el) : el > max && (max = el));
            }
            return (
              min === 1 / 0 ? (min = max) : max === -(1 / 0) && (max = min),
              (min !== 1 / 0 && min !== -(1 / 0)) ||
                ((min = null), (max = null)),
              [min, max]
            );
          ...
          function indicesOfNearest(a, x) {
            ...
            var lo = -1,
              hi = a.length,
              ascending = 1 === a.length || a[0] < a[1];
            if (ascending)
              for (; hi - lo > 1; ) {
                var mid = Math.round((lo + hi) / 2);
                a[mid] <= x ? (lo = mid) : (hi = mid);
              }
            else
              for (; hi - lo > 1; ) {
                var _mid = Math.round((lo + hi) / 2);
                a[_mid] >= x ? (lo = _mid) : (hi = _mid);
              }
            return (
              a[lo] === x && (hi = lo),
              -1 === lo && (lo = hi),
      ...
      10: [
        function (require, module, exports) {
          ...
          function matchesFilter(domain, filter) {
            ...
            try {
              for (
                var _iterator2 = Object.keys(filter)[Symbol.iterator](), _step2;
                !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next())
                  .done);
                _iteratorNormalCompletion2 = !0
              ) {
                var axisName = _step2.value,
                  condition = filter[axisName];
                ...
                if ("number" != typeof min && "string" != typeof min)
                ...
                if ((0, _referencing.isISODateAxis)(domain, axisName)) {
                  var _ref = ([
                      (0, _referencing.asTime)(min),
                      (0, _referencing.asTime)(max),
                    ][stop] = [
                      (0, _referencing.asTime)(start),
                      (0, _referencing.asTime)(stop),
                    ]),
                    _ref2 = _slicedToArray(_ref, 2);
                  (min = _ref2[0]), (max = _ref2[1]);
                } else if ((0, _referencing.isLongitudeAxis)(domain, axisName))
                  var lonWrapper = ((0, _referencing.getLongitudeWrapper)(
                    domain,
                    axisName
                  )[stop] = [lonWrapper(start), lonWrapper(stop)]);
                if (min > max) {
                  var _ref3 = [max, min];
                  (min = _ref3[0]), (max = _ref3[1]);
                }
                if (start > max || min > stop) return !1;
              }
            } catch (err) {
              (_didIteratorError2 = !0), (_iteratorError2 = err);
            } finally {
          ...
          function mergeInto(inputObj, targetObj) {
            ...
            try {
              for (
                var _iterator3 = Object.keys(inputObj)[Symbol.iterator](),
                  _step3;
                !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next())
                  .done);
                _iteratorNormalCompletion3 = !0
          ...
          var _constants = require("../constants.js"),
            _referencing = require("../domain/referencing.js"),
            CollectionQuery = (exports.CollectionQuery = (function () {
                ...
                _createClass(CollectionQuery, [
                  {
                    ...
                    value: function filter(spec) {
                      return mergeInto(spec, this._filter), this;
                    },
                  },
                  {
                    key: "subset",
                    value: function subset(spec) {
                      return mergeInto(spec, this._subset), this;
                    },
                  },
                  {
                    key: "execute",
                          ...
                          var _loop = function _loop() {
                              ...
                              promises.push(
                                cov.loadDomain().then(function (domain) {
                                  return matchesFilter(domain, _this._filter)
                                    ? 0 !== Object.keys(_this._subset).length
                                      ? cov
                                          .subsetByValue(_this._subset)
                                          .then(function (subsetted) {
                                            newcoll.coverages.push(subsetted);
                                          })
                                      : void newcoll.coverages.push(cov)
                                    : void 0;
                                })
                              );
                            },
                            _iterator = coll.coverages[Symbol.iterator](),
                            _step;
        ...
        { "../constants.js": 11, "../domain/referencing.js": 13 },
      ],
      11: [
        function (require, module, exports) {
          "use strict";
          ...
          var DOMAIN = (exports.DOMAIN = "Domain"),
            ...
            COVERAGECOLLECTION = (exports.COVERAGECOLLECTION =
              COVERAGE + "Collection"),
            COVJSON_PREFIX = "http://coveragejson.org/def#",
            COVJSON_DATATYPE_TUPLE = (exports.COVJSON_DATATYPE_TUPLE =
              COVJSON_PREFIX + "Tuple"),
            COVJSON_DATATYPE_POLYGON = (exports.COVJSON_DATATYPE_POLYGON =
              COVJSON_PREFIX + "Polygon");
        },
        {},
      ],
      12: [
        function (require, module, exports) {
          ...
          function subsetByBbox(cov, bbox) {
            ...
            return cov.subsetByValue(
              ((_cov$subsetByValue = {}),
              _defineProperty(_cov$subsetByValue, axes[0], {
                start: xmin,
                stop: xmax,
              }),
          ...
          function subsetByIndex(cov, constraints) {
            return cov.loadDomain().then(function (domain) {
              ...
              var newdomain = (0, _subset.subsetByIndex)(domain, constraints),
                rangeWrapper = function rangeWrapper(range) {
                  var newrange = {
                    dataType: range.dataType,
                    get: function get(obj) {
                      ...
                      try {
                        for (
                          var _iterator = Object.keys(obj)[Symbol.iterator](),
                            _step;
                          !(_iteratorNormalCompletion = (_step =
                            _iterator.next()).done);
                          _iteratorNormalCompletion = !0
                          ...
                          var axisName = _step.value,
                            _constraints$axisName = constraints[axisName],
                            start = _constraints$axisName.start,
                            step = _constraints$axisName.step;
                  ...
                  try {
                    for (
                      var _iterator2 = domain.axes.keys()[Symbol.iterator](),
                        _step2;
                      !(_iteratorNormalCompletion2 = (_step2 =
                        _iterator2.next()).done);
                      _iteratorNormalCompletion2 = !0
                      ...
                      var axisName = _step2.value,
                        size = newdomain.axes.get(axisName).values.length;
                ...
                loadRanges = function loadRanges(keys) {
                  return cov.loadRanges(keys).then(function (ranges) {
                    return new Map(
                      []
                        .concat(_toConsumableArray(ranges))
                        .map(function (_ref) {
          ...
          function subsetByValue(cov, constraints) {
            return cov.loadDomain().then(function (domain) {
              ...
              try {
                for (
                  var _iterator3 = Object.keys(constraints)[Symbol.iterator](),
                    _step3;
                  !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next())
                    .done);
                  _iteratorNormalCompletion3 = !0
                  ...
                  var axisName = _step3.value,
                    spec = constraints[axisName];
                  if (
                    void 0 !== spec &&
                    null !== spec &&
                    domain.axes.has(axisName)
                  ) {
                    var axis = domain.axes.get(axisName),
                      vals = axis.values,
                      isISODate = (0, _referencing.isISODateAxis)(
                        domain,
                        axisName
                      ),
                      isLongitude = (0, _referencing.isLongitudeAxis)(
                        domain,
                        axisName
                      ),
                      lonWrapper = isLongitude
                        ? (0, _referencing.getLongitudeWrapper)(
                            domain,
                            axisName
                          )
                        : void 0;
                    if (
                      ...
                      isISODate
                        ...
                        : isLongitude && (match = lonWrapper(match));
                      var i = void 0;
                      if (
                        ((i = vals.indexOf
                          ? vals.indexOf(match)
                          : Array.prototype.indexOf.call(vals, match)),
                        -1 === i)
                      )
                      ...
                      else if (isLongitude) target = lonWrapper(target);
                      else if (
                        "number" != typeof vals[0] ||
                        "number" != typeof target
                      )
                        throw new Error(
                          "Invalid axis or constraint value type"
                        );
                      var _i = (0, _array.indexOfNearest)(vals, target);
                      indexConstraints[axisName] = _i;
                    } else {
                      if (!("start" in spec && "stop" in spec))
                      ...
                      var _indicesOfNearest = (0, _array.indicesOfNearest)(
                          vals,
                          start
                        ),
                        _indicesOfNearest2 = _slicedToArray(
                          _indicesOfNearest,
                          2
                        ),
                        lo1 = _indicesOfNearest2[0],
                        hi1 = _indicesOfNearest2[1],
                        _indicesOfNearest3 = (0, _array.indicesOfNearest)(
                          vals,
                          stop
                        ),
                        _indicesOfNearest4 = _slicedToArray(
                          _indicesOfNearest3,
                          2
                        ),
                        lo2 = _indicesOfNearest4[0],
                        hi2 = _indicesOfNearest4[1],
                        imin = Math.min(lo1, hi1, lo2, hi2),
                        imax = Math.max(lo1, hi1, lo2, hi2) + 1;
          ...
          var _referencing = require("../domain/referencing.js"),
            _subset = require("../domain/subset.js"),
            _array = require("../array.js"),
            _constants = require("../constants.js");
        },
        {
          "../array.js": 9,
          "../constants.js": 11,
          "../domain/referencing.js": 13,
          "../domain/subset.js": 14,
        },
      ],
      13: [
        function (require, module, exports) {
          ...
          function getReferenceObject(domain, component) {
            var ref = domain.referencing.find(function (ref) {
              return -1 !== ref.components.indexOf(component);
            });
            return ref;
          }
          function getHorizontalCRSReferenceObject(domain) {
            var isHorizontal = function isHorizontal(ref) {
                return (
                  -1 !==
                    ["GeodeticCRS", "ProjectedCRS"].indexOf(ref.system.type) &&
                  2 === ref.components.length
                );
              },
              ref = domain.referencing.find(isHorizontal);
            return ref;
          }
          function isEllipsoidalCRS(rs) {
            return -1 !== EllipsoidalCRSs.indexOf(rs.id);
          }
            ...
            if (ref) {
              ...
              if (domain.axes.has(lonComponent)) {
                ...
                if (
                  ...
                  (lonMin = _ref[0]), (lonMax = _ref[1]);
                }
              } else {
                var axes = [].concat(_toConsumableArray(domain.axes.values())),
                  axis = axes.find(function (axis) {
                    return -1 !== axis.components.indexOf(lonComponent);
                  }),
                  lonCompIdx = axis.components.indexOf(lonComponent);
                if (
                  ((lonMin = 1 / 0),
                  (lonMax = -(1 / 0)),
                  axis.dataType === _constants.COVJSON_DATATYPE_TUPLE)
                ) {
                  var _iteratorNormalCompletion = !0,
                    _didIteratorError = !1,
                    _iteratorError = void 0;
                  try {
                    for (
                      var _iterator = axis.values[Symbol.iterator](), _step;
                      !(_iteratorNormalCompletion = (_step = _iterator.next())
                        .done);
                      _iteratorNormalCompletion = !0
                    ) {
                      var tuple = _step.value,
                        lon = tuple[lonCompIdx];
                  ...
                  if (axis.dataType !== _constants.COVJSON_DATATYPE_POLYGON)
                  ...
                  try {
                    for (
                      var _iterator2 = axis.values[Symbol.iterator](), _step2;
                      !(_iteratorNormalCompletion2 = (_step2 =
                        _iterator2.next()).done);
                      _iteratorNormalCompletion2 = !0
                    ) {
                      ...
                      try {
                        for (
                          var _iterator3 = poly[Symbol.iterator](), _step3;
                          !(_iteratorNormalCompletion3 = (_step3 =
                            _iterator3.next()).done);
                          _iteratorNormalCompletion3 = !0
                        ) {
                          ...
                          try {
                            for (
                              var _iterator4 = ring[Symbol.iterator](), _step4;
                              !(_iteratorNormalCompletion4 = (_step4 =
                                _iterator4.next()).done);
                              _iteratorNormalCompletion4 = !0
                            ) {
                              var point = _step4.value,
                                _lon = point[lonCompIdx];
              ...
              var lonMid = (lonMax + lonMin) / 2,
                lonMinExtended = lonMid - 180,
                lonMaxExtended = lonMid + 180;
              return {
                project: function project(_ref2) {
                  var lon = _ref2.lon,
                    ...
                    lonProjected = void 0;
                  lonProjected =
                    lon >= lonMinExtended && lonMaxExtended >= lon
                      ? lon
                      : ((((lon - lonMinExtended) % 360) + 360) % 360) +
                        lonMinExtended;
                  var _ref3 =
                      0 === lonIdx ? [lonProjected, lat] : [lat, lonProjected],
          ...
          function getLongitudeWrapper(domain, axisName) {
            ...
            var vals = domain.axes.get(axisName).values,
              lon_min = vals[0],
              lon_max = vals[vals.length - 1];
            if (lon_min > lon_max) {
              var _ref8 = [lon_max, lon_min];
              (lon_min = _ref8[0]), (lon_max = _ref8[1]);
            }
            var x_mid = (lon_max + lon_min) / 2,
              x_min = x_mid - 180,
              x_max = x_mid + 180;
            return function (lon) {
              return lon >= x_min && x_max >= lon
                ? lon
                : ((((lon - x_min) % 360) + 360) % 360) + x_min;
            };
          }
          function isLongitudeAxis(domain, axisName) {
            ...
            if (-1 === EllipsoidalCRSs.indexOf(crsId)) return !1;
            var compIdx = ref.components.indexOf(axisName),
              isLongitude = LongitudeAxisIndex[crsId] === compIdx;
            return isLongitude;
          }
          ...
          (exports.getReferenceObject = getReferenceObject),
            (exports.getHorizontalCRSReferenceObject =
              getHorizontalCRSReferenceObject),
          ...
          var _constants = require("../constants.js"),
            ...
            LongitudeAxisIndex =
              ((_LongitudeAxisIndex = {}),
              _defineProperty(_LongitudeAxisIndex, EPSG4979, 1),
              _defineProperty(_LongitudeAxisIndex, EPSG4326, 1),
              _defineProperty(_LongitudeAxisIndex, CRS84, 0),
              _LongitudeAxisIndex);
        },
        { "../constants.js": 11 },
      ],
        ...
        function (require, module, exports) {
          ...
          function normalizeIndexSubsetConstraints(domain, constraints) {
            ...
            for (var axisName in constraints)
              if (
                domain.axes.has(axisName) &&
                void 0 !== constraints[axisName] &&
                null !== constraints[axisName]
              ) {
                if ("number" == typeof constraints[axisName]) {
                  var constraint = constraints[axisName];
                  normalizedConstraints[axisName] = {
                    start: constraint,
                    stop: constraint + 1,
                  };
                } else normalizedConstraints[axisName] = constraints[axisName];
                var _normalizedConstraint = normalizedConstraints[axisName],
                  _normalizedConstraint2 = _normalizedConstraint.start,
                  start =
                    void 0 === _normalizedConstraint2
                      ? 0
                      : _normalizedConstraint2,
                  _normalizedConstraint3 = _normalizedConstraint.stop,
                  stop =
                    void 0 === _normalizedConstraint3
                      ? domain.axes.get(axisName).values.length
                      : _normalizedConstraint3,
                  _normalizedConstraint4 = _normalizedConstraint.step,
                  step =
                    void 0 === _normalizedConstraint4
                      ? 1
                      : _normalizedConstraint4;
                if (0 >= step)
                  throw new Error(
            ...
            try {
              for (
                var _iterator = domain.axes.keys()[Symbol.iterator](), _step;
          ...
          function subsetByIndex(domain, constraints) {
            ...
            try {
              for (
                var _loop = function _loop() {
                    var axisName = _step2.value,
                      axis = domain.axes.get(axisName),
                      coords = axis.values,
                      bounds = axis.bounds,
                      constraint = constraints[axisName],
                      newcoords = void 0,
                      newbounds = void 0,
                      start = constraint.start,
                      stop = constraint.stop,
                      step = constraint.step;
                    if (0 === start && stop === coords.length && 1 === step)
                      (newcoords = coords), (newbounds = bounds);
                    else if (1 === step)
                      (newcoords = coords.subarray
                        ? coords.subarray(start, stop)
                        : coords.slice(start, stop)),
                        bounds &&
                          (newbounds = {
                            get: function get(i) {
                      ...
                      for (var i = start, j = 0; stop > i; i += step, j++)
                  ...
                  _iterator2 = Object.keys(constraints)[Symbol.iterator](),
                  _step2;
                !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next())
                  .done);
                _iteratorNormalCompletion2 = !0
          ...
          Object.defineProperty(exports, "__esModule", { value: !0 }),
            (exports.normalizeIndexSubsetConstraints =
              normalizeIndexSubsetConstraints),
            (exports.subsetByIndex = subsetByIndex);
          var _constants = require("../constants.js");
        },
        { "../constants.js": 11 },
      ...
      15: [
        function (require, module, exports) {
          ...
          function compileConstructor(dtype, dimension) {
            ...
            0 > dimension && (className = "View_Nil" + dtype);
            var useGetters = "generic" === dtype;
            if (-1 === dimension) {
              var code =
                  "function " +
                  className +
            ...
            if (0 === dimension) {
              var code =
                  ...
                  className +
                  "(a,d)}",
                procedure = new Function("TrivialArray", code);
              return procedure(CACHED_CONSTRUCTORS[dtype][0]);
            }
            var code = ["'use strict'"],
              indices = iota(dimension),
              args = indices.map(function (i) {
                return "i" + i;
              }),
              index_str =
                "this.offset+" +
          ...
          function arrayDType(data) {
            ...
            return Array.isArray(data) ? "array" : "generic";
          }
          function wrappedNDArrayCtor(data, shape, stride, offset) {
            ...
            "number" == typeof data && (data = [data]),
              void 0 === shape && (shape = [data.length]);
            var d = shape.length;
            if (void 0 === stride) {
              stride = new Array(d);
            ...
            if (void 0 === offset) {
              offset = 0;
              for (var i = 0; d > i; ++i)
                stride[i] < 0 && (offset -= (shape[i] - 1) * stride[i]);
            }
            for (
              var dtype = arrayDType(data),
                ctor_list = CACHED_CONSTRUCTORS[dtype];
              ctor_list.length <= d + 1;
        ...
        { "iota-array": 16, "is-buffer": 17 },
      ],
      16: [
        function (require, module, exports) {
          "use strict";
          function iota(n) {
            for (var result = new Array(n), i = 0; n > i; ++i) result[i] = i;
            return result;
          }
          module.exports = iota;
        },
            ...
            return (
              ...
              (UrlTemplate.prototype.encodeValue = function (
                ...
                return (
                  ...
                  key ? this.encodeUnreserved(key) + "=" + value : value
                );
              }),
                  ...
                  if (
                    ...
                    Array.isArray(value)
                      ...
                      this.isKeyOperator(operator)
                        ...
                        : 0 !== tmp.length && result.push(tmp.join(","));
                  }
                else
                  ";" === operator
                      ...
                      function (_, expression, literal) {
                        if (expression) {
                          var operator = null,
                            values = [];
                          if (
                            (-1 !== operators.indexOf(expression.charAt(0)) &&
