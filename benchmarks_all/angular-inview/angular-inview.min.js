(function () {
      ...
      function ($parse) {
        return {
          ...
          link: function (scope, element, attrs, containerController) {
            ...
            performCheck =
              (ref =
                (ref1 = item.customDebouncedCheck) != null
                  ? ref1
  ...
  bindWindowEvents = function () {
    ...
    return angular
      .element(window)
      .bind(
        "checkInView click ready wheel mousewheel DomMouseScroll MozMousePixelScroll resize scroll touchmove mouseup",
        windowEventsHandler
      );
  };
  ...
  triggerInViewCallback = function (
    event,
    item,
    inview,
    isTopVisible,
    ...
    if (inview) {
      elOffsetTop =
        getBoundingClientRect(item.element[0]).top + window.pageYOffset;
      inviewpart =
        (isTopVisible && isBottomVisible && "neither") ||
        (isTopVisible && "top") ||
  ...
  checkInView = function (items, container, event) {
    var bounds,
      ...
      results,
      viewport;
    viewport = { top: 0, bottom: getViewportHeight() };
    ...
    for (k = 0, len1 = items.length; k < len1; k++) {
      ...
      boundsTop =
        bounds.top +
        (offsetIsPercentage(item.offset)
          ? getOffsetFromPercentage(bounds, item.offset)
          : parseInt(
              (ref = (ref1 = item.offset) != null ? ref1[0] : void 0) != null
                ? ref
                : item.offset
            ));
      boundsBottom =
        ...
        (offsetIsPercentage(item.offset)
          ? getOffsetFromPercentage(bounds, item.offset)
          : parseInt(
              (ref2 = (ref3 = item.offset) != null ? ref3[1] : void 0) != null
                ? ref2
                : item.offset
            ));
      if (boundsTop < viewport.bottom && boundsBottom >= viewport.top) {
        results.push(
          triggerInViewCallback(
            ...
            item,
            true,
            boundsBottom > viewport.bottom,
            boundsTop < viewport.top
          )
        );
      } else {
