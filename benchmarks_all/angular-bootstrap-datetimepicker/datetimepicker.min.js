(function (factory) {
  ...
  function DatetimepickerDirective(defaultConfig, configurationValidator) {
    ...
    function DirectiveController($scope, $element, $attrs) {
      ...
      function yearModelFactory(milliseconds) {
        ...
        var startDate = moment
          .utc(startOfDecade(milliseconds))
          .subtract(1, "year")
          .startOf("year");
      ...
      function dayModelFactory(milliseconds) {
        ...
        var startDate = moment
          .utc(startOfMonth)
          .subtract(Math.abs(startOfMonth.weekday()), "days");
        var activeDate = formatValue(
      ...
      function minuteModelFactory(milliseconds) {
        ...
        for (var i = 0; i < limit; i += 1) {
          var hourMoment = moment
            .utc(selectedDate)
            .add(i * configuration.minuteStep, "minute");
          var dateValue = {
            utcDateValue: hourMoment.valueOf(),
            display: hourMoment.format("LT"),
      ...
      function getMoment(modelValue) {
        return moment(
          modelValue,
          angular.isString(modelValue) ? configuration.parseFormat : undefined
        );
      }
      function getUTCTime(modelValue) {
        ...
        return tempDate.getTime() - tempDate.getTimezoneOffset() * 6e4;
      }
    ...
    function DateObject() {
      var tempDate = new Date();
      var localOffset = tempDate.getTimezoneOffset() * 6e4;
  ...
  function DateTimePickerValidatorService($log) {
    ...
    function validator(configuration) {
      ...
      if (
        validViews.indexOf(configuration.minView) >
        validViews.indexOf(configuration.startView)
      ) {
      ...
      if (
        configuration.configureOn !== null &&
        !angular.isString(configuration.configureOn)
      ) {
        throw "configureOn must be a string";
      }
      if (
        configuration.configureOn !== null &&
        configuration.configureOn.length < 1
      ) {
        throw "configureOn must not be an empty string";
      }
      if (
        configuration.renderOn !== null &&
        !angular.isString(configuration.renderOn)
      ) {
        throw "renderOn must be a string";
      }
      if (
        configuration.renderOn !== null &&
        configuration.renderOn.length < 1
      ) {
        throw "renderOn must not be an empty string";
      }
      if (
        configuration.modelType !== null &&
        !angular.isString(configuration.modelType)
      ) {
        throw "modelType must be a string";
      }
      if (
        configuration.modelType !== null &&
        configuration.modelType.length < 1
      ) {
        throw "modelType must not be an empty string";
      }
      if (
        configuration.modelType !== "Date" &&
        configuration.modelType !== "moment" &&
        configuration.modelType !== "milliseconds"
      ) {
        configuration.parseFormat = configuration.modelType;
      }
      if (
        configuration.dropdownSelector !== null &&
        !angular.isString(configuration.dropdownSelector)
      ) {
      ...
      if (
        ...
        (typeof jQuery === "undefined" ||
          typeof jQuery().dropdown !== "function")
      ) {
        $log.error(
