  ...
  var a = function (a) {
    ...
    function m(a, b) {
      ...
      while ((h = c[++e]))
        (g = h.indexOf("=")),
          (l = h.substring(0, g)),
          (j = decodeURIComponent(h.substring(g + 1))),
          b !== !1 && (j = k(j)),
          l in f ? (i(f[l]) ? f[l].push(j) : (f[l] = [f[l], j])) : (f[l] = j);
      return f;
    }
    function n() {
                ...
                for (var g in b[c]) {
                  ...
                  if (i(f))
                    for (var h in f)
                      g.slice(-2) == "[]"
                        ? e.push(
                            encodeURI(g.slice(0, -2)) + "[]=" + encodeURI(f[h])
                          )
                        : e.push(encodeURI(g + "=" + f[h]));
                  else e.push(encodeURI(g + "=" + f));
                }
                d = "?" + e.join("&");
              } else d = String(b[c]);
