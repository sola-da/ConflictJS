window.allowMe = (function () {
  function a() {}
  function b(a, b, d) {
    var e = document.createElement("img");
    (e.src = a), (e.style.visibility = "hidden"), document.body.appendChild(e);
    var f = document.createElement("canvas");
    (f.width = e.offsetWidth), (f.height = e.offsetHeight);
    var g = f.getContext("2d");
    g.drawImage(e, 0, 0), e.parentNode.removeChild(e);
    for (
      var h,
        i,
        j,
        k = g.getImageData(0, 0, f.width, f.height),
        a = k.data,
        l = c(b),
        m = c(d),
        n = 0,
        o = a.length;
      o > n;
      n += 4
    )
      (h = a[n]),
        (i = a[n + 1]),
        (j = a[n + 2]),
        h == l.r &&
          i == l.g &&
          j == l.b &&
          ((a[n] = m.r), (a[n + 1] = m.g), (a[n + 2] = m.b));
    return g.putImageData(k, 0, 0), f.toDataURL();
  }
  function c(a) {
    var b = {};
    return (
      (b.r = parseInt(a.substr(1, 2), 16)),
      (b.g = parseInt(a.substr(3, 2), 16)),
      (b.b = parseInt(a.substr(5, 2), 16)),
      b
    );
  }
  var d, e;
  return (
    (a.prototype = {
      show: function (a, c) {
        e && this.hide(), (e = !0), (d = document.getElementById(a));
        var f = "";
        c.backdropColor && (f = "background:" + c.backdropColor + ";"),
          c.backdropOpacity && (f += "opacity:" + c.backdropOpacity + ";");
        var g = "";
        c.headerFontSize && (g = "font-size:" + c.headerFontSize + ";"),
          c.headerFont && (g += "font-family:" + c.headerFont + ";");
        var h = "";
        c.captionFontSize && (h = "font-size:" + c.captionFontSize + ";"),
          c.captionFont && (h += "font-family:" + c.captionFont + ";");
        var i = "";
        if (
          (c.arrowColor &&
            ((i = b(
              "data:image/png;base64,",
              "#FFFCFF",
              c.arrowColor
            )),
            (i = "background-image: url(" + i + ");")),
          c.arrowSize)
        ) {
          var j = 100 * c.arrowSize,
            k = 30 * c.arrowSize;
          i += "height:" + j + "px; width:" + k + "px;";
        }
        (d.innerHTML =
          '<div class="container" style="' +
          f +
          '" > </div><div class="content"> <div class="left arrow bounce" style="' +
          i +
          '" >  </div> <div class="right arrow bounce" style="' +
          i +
          '" >  </div> <div class="text-content"> <div class="header-text" style="' +
          g +
          '" > ' +
          c.headerText +
          '</div><div id="allowMe-caption" class="caption-text" style="' +
          h +
          '" > '),
          c.captionText &&
            (document.getElementById("allowMe-caption").innerHTML =
              c.captionText),
          console.log(d.innerHTML);
      },
      hide: function () {
        (d.innerHTML = ""), (e = !1);
      },
    }),
    a
  );
})(window);
