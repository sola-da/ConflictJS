...
(function (global, factory) {
  if (typeof define === "function" && define.amd) {
    ...
    return (global["BarinBritva.CoordinatesPicker"] = factory(
      global.jQuery,
      global.google
    ));
  }
  ...
  BarinBritva.CoordinatesPicker.prototype = {
    ...
    _defaultZoom: 4,
    _zoomLevels: { country: 4, locality: 10, route: 15, street_address: 17 },
    _mapParams: { mapTypeId: google.maps.MapTypeId.ROADMAP },
    _markerParams: { draggable: true, animation: google.maps.Animation.DROP },
    _selectValue: false,
    _map: null,
    ...
    _setParams: function (params) {
      ...
      for (var key in necessary) {
        ...
        if (!(option instanceof jQuery) || option.size() == 0) {
          throw new Error(
            'Parameter "' + necessary[key] + '" is empty or not jQuery object.'
          );
        }
      }
      ...
      if (typeof params.center !== "undefined") {
        if (params.center instanceof google.maps.LatLng) {
          ...
          if (params.center instanceof Array) {
            if (
              params.center.length !== 2 ||
              isNaN(params.center[0]) ||
              isNaN(params.center[1])
            ) {
              throw new Error('Parameter "center" must have only two numbers.');
            }
            this._defaultCoordinates = new google.maps.LatLng(
              params.center[0],
              params.center[1]
            );
          } else {
            throw new Error(
              'Parameter "center" is not google.maps.LatLng object or array.'
            );
          }
        }
      }
      if (typeof params.zooms !== "undefined") {
        if (params.zooms instanceof Object) {
          ...
          if (
            typeof zooms.street_address !== "undefined" &&
            !isNaN(zooms.street_address)
          ) {
      ...
      if (
        typeof params.marker !== "undefined" &&
        params.marker instanceof Object
      ) {
    ...
    _putMarker: function (position) {
      ...
      if (
        marker === null ||
        !this._isEqualCoordinates(marker.getPosition(), position)
      ) {
        this._removeMarker();
    ...
    _burnCoordinatesChange: function () {
      var marker = this._marker;
      var data = { type: "coordschange", marker: marker };
    ...
    _geocode: function (address, callback) {
      ...
      var self = this;
      this._burnEvent({ type: "geocodingstart", address: address });
