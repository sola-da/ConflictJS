...
!(function (window, document) {
  var absPos = function (element) {
    ...
    ProgressCircle = function (params) {
      (this.canvas = params.canvas),
        ...
        (this.infoLineAngleInterval =
          params.infoLineAngleInterval || Math.PI / 8),
      ...
      return (
        this.circles.push(
          new Circle({
            ...
            innerRadius:
              this.minRadius +
              this.circles.length * (this.gapWidth + this.arcWidth),
  ...
  var Circle = function (params) {
    if (
      ...
      (this.infoLineStartX = this.centerX + sinA * arcDistance),
        ...
        (this.infoLineEndX =
          this.infoLineMidX +
          (0 > sinA ? -this.horizLineLength : this.horizLineLength)),
  ...
  (Circle.prototype = {
    ...
    update: function () {
      (this.progress = this.progressListener()),
        this._draw(),
        this.infoListener &&
          ((this.info = this.infoListener()), this._drawInfo());
    },
    _draw: function () {
    ...
    _drawInfo: function () {
      ...
      return (
        ...
        (this.infoText.style.top =
          this.infoLineEndY + absPos(this.canvas)[1] - lineHeight / 2 + "px"),
        this
      );
    },
  ...
  jQuery(function ($) {
    var colors = [
      ...
      Alert = { ERROR: "alert-danger", WARNING: "alert-warning" },
      ...
      redirectToNewJobConsole = function (jobUrl, buildNumber) {
        return isRootHomepage(jobUrl)
          ? void $.getJSON(
              jobUrl + "api/json?tree=activeConfigurations[name]",
              function (data) {
                if (
                  "{}" !== JSON.stringify(data) &&
                  "activeConfigurations" in data
                ) {
                  var downstreamUrl =
                    jobUrl + data.activeConfigurations[0].name + "/";
    ...
    var getCallout = function (message, href) {
        return (
          "<div class='doony-callout doony-callout-info'><a " +
          (null === href ? "" : "href='" + href + "'") +
          ">" +
          message +
          "</a></div>"
      ...
      updateConfiguration = function (jobUrl, name) {
        $.getJSON(
          jobUrl + name + "api/json?tree=lastBuild[number]",
          function (data) {
            null !== data.lastBuild &&
              "number" in data.lastBuild &&
              $("#matrix .model-link").each(function (_, item) {
                if (item.getAttribute("href") === name) {
                  var href =
                    jobUrl + name + data.lastBuild.number + "/consoleFull";
                  $(item)
                    .next(".doony-callout")
                    .children("a")
                    .attr("href", href);
    ...
    $("#matrix").length &&
      setInterval(function () {
        ...
        $("#matrix .doony-downstream-link").length ||
    ...
    var replaceFloatyBall = function (selector, type) {
        $(selector).each(function () {
          ...
          var dimension;
          "48" === this.getAttribute("width") ||
          "24" === this.getAttribute("width")
            ? ((dimension = 0.5 * this.getAttribute("width") + 8),
              (wrapper.style.marginRight = "15px"),
              (wrapper.style.verticalAlign = "middle"))
            : this.classList.contains("icon32x32")
            ? ((dimension = 24),
    ...
    if (
      ...
      isJobHomepage(window.location.pathname))
    ) {
      var jobUrl = getJobUrl(window.location.pathname);
    ...
    if (isJobPage(window.location.pathname)) {
      ...
      (button.className = "btn btn-primary doony-build"),
        ...
        $(button).click(function () {
          ...
          $.getJSON(
            jobUrl +
              "api/json?depth=1&tree=nextBuildNumber,lastBuild[building]",
            function (data) {
              $.post(jobUrl + "build", function () {
                var message =
                  "Build #" +
                  data.nextBuildNumber +
                  " created, you will be redirected when it is ready.";
                "{}" !== JSON.stringify(data) &&
                  "lastBuild" in data &&
                  null !== data.lastBuild &&
                  data.lastBuild.building &&
                  (message +=
                    " <a href='#' id='doony-clear-build'>Cancel the current build</a>"),
                  showButterBar(message, Alert.WARNING),
                  redirectToNewJobConsole(
                    getJobUrl(window.location.pathname),
                    data.nextBuildNumber
                  );
              }).fail(function (jqXHR) {
                403 === jqXHR.status
                  ? showButterBar(
                      "Cannot create build. Maybe you need to log in or have the 'build' permission.",
                      Alert.ERROR
                    )
                  : showButterBar(
        ...
        $(document).on("click", "#doony-clear-build", function (e) {
          ...
          var jobUrl = getRootJobUrl(window.location.pathname);
          $.getJSON(
            jobUrl + "api/json?tree=lastBuild[number]",
            function (data) {
      ...
      title.children("div").length
        ? title.append(button)
        : (title.css("display", "inline-block"), title.after(button));
    }
    $("#l10n-footer").after(
      "<span class='doony-theme'>Browsing Jenkins with the <a target='_blank' href='https://github.com/kevinburke/doony'>Doony theme</a></span>"
    );
  });
